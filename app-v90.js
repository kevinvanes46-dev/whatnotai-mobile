...
const STORAGE_PREFS = 'cardscout_mobile_prefs_v80';
const CATALOG_CACHE_KEY = 'cardscout_dp_pop_catalog_v80';
const MAX_RECENT = 10;
const MAX_FAV = 40;
...
function detectSet(text){
  ...
  const candidates=SET_ALIASES.flatMap(([set,aliases]) => (aliases||[]).map(alias=>({set,alias})))
    .sort((a,b)=>b.alias.length-a.alias.length);
  for(const item of candidates){ if(lower.includes(' '+item.alias+' ')) return item.set; }
  return 'AUTO';
}
...
function buildSearchTerm(name, number, set){
  ...
  const n = cleanNumber(number);
  // v80: for DP/POP search-only catalog entries prefer the full set name over a guessed product slug.
  if(def?.safeSearchLabel && n) return `${name} ${def.safeSearchLabel} ${n}`;
...
function clearAll(){
  quickInput.value=''; numberInput.value=''; nameInput.value=''; setSelect.value='AUTO'; if(editionSelect) editionSelect.value='AUTO';
  updateCustomSelects();
...
// ---------------- v85 FOCUSED SCAN: preferences + Smart Search + cached DP/POP catalog ----------------
...
const CATALOG_PACKS = {
  ...
  'POP SERIES 1':'pop1','POP SERIES 2':'pop2','POP SERIES 3':'pop3',
  'POP SERIES 4':'pop4','POP SERIES 5':'pop5','POP SERIES 6':'pop6',
  'POP SERIES 7':'pop7','POP SERIES 8':'pop8','POP SERIES 9':'pop9'
};

function savePreferences(){
  try{
    localStorage.setItem(STORAGE_PREFS, JSON.stringify({lang:langSelect.value,cond:condSelect.value}));
  }catch(e){}
}
function loadPreferences(){
  try{
    const p=JSON.parse(localStorage.getItem(STORAGE_PREFS)||'{}');
    if(['JP','EN'].includes(p.lang)) langSelect.value=p.lang;
    if(['NM','EX','GD','PL'].includes(p.cond)) condSelect.value=p.cond;
  }catch(e){}
}
function updateCatalogBadges(){
  const total=(DATA.knownCards||[]).length;
  const sets=Object.keys(DATA.sets||{}).length;
  if(dbBadge) dbBadge.textContent=`DB ${total}`;
  if(setBadge) setBadge.textContent=`SETS ${sets}`;
  if(catalogBadge) catalogBadge.textContent=catalogPackReady?'DP+POP READY':'DP+POP';
}
function catalogKey(c){ return `${String(c.language||'EN').toLowerCase()}|${String(c.set||'').toLowerCase()}|${String(c.number||'')}|${normalizeName(c.name)}`; }
function mergeCatalogRecords(records=[]){
  if(!Array.isArray(records)||!records.length) return 0;
  const seen=new Set((DATA.knownCards||[]).map(c=>c.key||catalogKey(c)));
  let added=0;
  for(const c of records){
    const key=c.key||catalogKey(c);
    if(seen.has(key)) continue;
    c.key=key; seen.add(key); DATA.knownCards.push(c); added++;
  }
  DATA.stats.total=DATA.knownCards.length;
  DATA.stats.direct=DATA.knownCards.filter(c=>!!c.url).length;
  DATA.stats.searchOnly=DATA.stats.total-DATA.stats.direct;
  DATA.stats.sets=Object.keys(DATA.sets||{}).length;
  return added;
}
function compactPackRecord(raw,setKey){
  const def=DATA.sets[setKey]||{};
  const number=String(raw?.number||'').trim();
  const name=String(raw?.name||'').trim();
  if(!name||!number) return null;
  return {
    key:`en|${setKey.toLowerCase()}|${number}|${normalizeName(name)}`,
    name, number, set:setKey, set_name:def.label||setKey, code:def.code||'', language:'EN',
    rarity:String(raw?.rarity||raw?.supertype||'').trim(), url:'',
    query:`${name} ${def.safeSearchLabel||def.label||setKey} ${number}`,
    aliases:[], verified:false, direct:false, catalogPack:'dp-pop-v80'
  };
}
function loadCachedCatalogPack(){
  try{
    const cached=JSON.parse(localStorage.getItem(CATALOG_CACHE_KEY)||'null');
    if(cached?.version==='v80' && Array.isArray(cached.records) && cached.records.length>500){
      mergeCatalogRecords(cached.records);
      catalogPackReady=true;
      updateCatalogBadges();
      return true;
    }
  }catch(e){}
...
async function fetchCatalogPack(){
  if(catalogPackReady) return;
  if(catalogBadge) catalogBadge.textContent='DP+POP…';
  const entries=Object.entries(CATALOG_PACKS);
  const all=[];
  // Four at a time keeps mobile Safari responsive and avoids a network burst.
  for(let i=0;i<entries.length;i+=4){
    const batch=entries.slice(i,i+4);
    const results=await Promise.allSettled(batch.map(async([setKey,id])=>{
      const url=`https://raw.githubusercontent.com/PokemonTCG/pokemon-tcg-data/master/cards/en/${id}.json`;
      const res=await fetch(url,{cache:'force-cache'});
      if(!res.ok) throw new Error(`${id}: ${res.status}`);
      const rows=await res.json();
      return (rows||[]).map(r=>compactPackRecord(r,setKey)).filter(Boolean);
    }));
    results.forEach(r=>{ if(r.status==='fulfilled') all.push(...r.value); });
    await new Promise(resolve=>setTimeout(resolve,30));
  }
  if(all.length>500){
    mergeCatalogRecords(all);
    catalogPackReady=true;
    try{ localStorage.setItem(CATALOG_CACHE_KEY,JSON.stringify({version:'v80',records:all})); }catch(e){}
    updateCatalogBadges();
    refreshSmartResults();
  }else if(catalogBadge){
    catalogBadge.textContent='DP+POP OFFLINE';
  }
}
function scheduleCatalogPack(){
  if(loadCachedCatalogPack()) return;
  // Give the scanner first priority on a fresh load. The catalog is background data;
  // it must never compete with the first Tesseract worker download/prewarm.
  const start=()=>{
    if(ocrBusy || ocrWorkerPromise){ setTimeout(start,2500); return; }
    fetchCatalogPack().catch(err=>{ console.warn('DP/POP catalog pack unavailable',err); if(catalogBadge) catalogBadge.textContent='DP+POP OFFLINE'; });
  };
  setTimeout(start,7500);
}

function smartParts(text){
  const original=String(text||'').trim();
  const work=' '+original.toLowerCase().replace(/\s+/g,' ')+' ';
  const lang=/\b(jp|jpn|japanese|japans)\b/.test(work)?'JP':(/\b(en|eng|english)\b/.test(work)?'EN':langSelect.value);
  const set=detectSet(original);
  const firstEdition=/\b(1st(?:\s+ed(?:ition)?)?|first\s+edition|edition\s*1|1e)\b/i.test(original);
  const unlimited=/\b(unlimited|unl)\b/i.test(original);
  let cond=condSelect.value || 'NM';
  const condMatch=original.match(/\b(near\s+mint|excellent|played|good|nm|gd|pl)\b/i);
  if(condMatch){
    const token=condMatch[1].toLowerCase().replace(/\s+/g,' ');
    if(token==='nm'||token==='near mint') cond='NM';
    else if(token==='excellent') cond='EX';
    else if(token==='gd'||token==='good') cond='GD';
    else if(token==='pl'||token==='played') cond='PL';
  }
  let cleaned=' '+removeSetWords(original)+' ';
  cleaned=cleaned.replace(/\b(jp|jpn|japanese|japans|en|eng|english|1st|1st\s+ed|1st\s+edition|first\s+edition|edition\s*1|1e|unlimited|unl)\b/gi,' ');
  cleaned=cleaned.replace(/\b(near\s+mint|excellent|played|good|nm|gd|pl)\b/gi,' ');
  // Bare "ex" is intentionally left in the name because cards such as Rayquaza ex exist.
  // For Excellent use the EX button or type "excellent"; this prevents false card-name stripping.
  const tokens=cleaned.trim().split(/\s+/).filter(Boolean);
  let number=''; const name=[];
  for(const t of tokens){
    if(!number && (/^\d{1,3}(?:\/\d{1,3})?$/.test(t)||/^[a-z]{1,3}\d{1,3}$/i.test(t))) number=t.split('/')[0];
    else name.push(t);
  }
  return {original,lang,set,number,nameQuery:normalizeName(name.join(' ')),cond,edition:firstEdition?'1ST':(unlimited?'AUTO':editionSelect.value)};
}
function smartCardScore(card,parts){
  if(card.language!==parts.lang) return -1;
  if(parts.set!=='AUTO' && card.set!==parts.set) return -1;
  const cn=normalizeName(card.name); const aliases=(card.aliases||[]).map(normalizeName);
  const hay=[cn,...aliases]; let score=0;
  if(parts.number){
    const a=String(card.number||'').toLowerCase(), b=String(parts.number).toLowerCase();
    if(a===b || cleanNumber(a)===cleanNumber(b)) score+=8;
    else if(a.startsWith(b)) score+=2;
    else return -1;
  }
  if(parts.nameQuery){
    let best=0;
    for(const h of hay){
      if(h===parts.nameQuery) best=Math.max(best,12);
      else if(h.startsWith(parts.nameQuery)) best=Math.max(best,9);
      else if(h.includes(parts.nameQuery)) best=Math.max(best,7);
      else {
        const qTokens=parts.nameQuery.split(' ').filter(Boolean);
        if(qTokens.length && qTokens.every(t=>h.includes(t))) best=Math.max(best,5);
      }
    }
    if(!best) return -1; score+=best;
  }
  if(parts.set!=='AUTO') score+=5;
  if(card.url) score+=.35;
  return score;
}
function smartMatches(text,limit=3){
  const parts=smartParts(text);
  if(!parts.number && parts.nameQuery.length<2) return [];
  return (DATA.knownCards||[])
    .map(c=>({card:c,score:smartCardScore(c,parts)}))
    .filter(x=>x.score>=0)
    .sort((a,b)=>b.score-a.score || Number(cleanNumber(a.card.number)||999)-Number(cleanNumber(b.card.number)||999))
    .slice(0,limit).map(x=>x.card);
}
function hideSmartResults(){ if(smartResults){ smartResults.hidden=true; smartResults.innerHTML=''; } }
function selectSmartCard(c){
  if(!c) return;
  const parts=smartParts(quickInput.value);
  quickInput.value=[c.number,c.name,(DATA.sets[c.set]?.label||c.set)].filter(Boolean).join(' ');
  numberInput.value=c.number; nameInput.value=c.name; setSelect.value=c.set;
  if(c.language) langSelect.value=c.language;
  if(parts.cond) condSelect.value=parts.cond;
  if(parts.edition) editionSelect.value=parts.edition;
  updateCustomSelects(); savePreferences(); hideSmartResults();
  makeLink(false);
}
function refreshSmartResults(){
  if(!smartResults) return;
  const matches=smartMatches(quickInput.value,3);
  if(!matches.length){ hideSmartResults(); return; }
  smartResults.innerHTML=matches.map((c,i)=>`<button type="button" class="smartResult" data-smart-index="${i}"><span><b>${escapeHtml(c.name)}</b> <em>#${escapeHtml(c.number)}</em></span><small>${escapeHtml(DATA.sets[c.set]?.label||c.set)}${c.url?' · direct':' · veilige zoekroute'}</small></button>`).join('');
  smartResults.hidden=false;
  smartResults.querySelectorAll('[data-smart-index]').forEach(btn=>btn.addEventListener('click',()=>selectSmartCard(matches[Number(btn.dataset.smartIndex)])));
}
function queueSmartResults(){ clearTimeout(smartTimer); smartTimer=setTimeout(refreshSmartResults,85); }
// ---------------- end v85 FOCUSED SCAN ----------------
...
function bind(){
  ...
  quickInput.addEventListener('input', queueSmartResults);
  quickInput.addEventListener('keydown', e => { if(e.key==='Escape'){ hideSmartResults(); return; } if(e.key==='Enter'){ e.preventDefault(); hideSmartResults(); processQuick(); }});
  ...
  [setSelect,langSelect,condSelect,editionSelect].filter(Boolean).forEach(el => el.addEventListener('change', () => { if(el===langSelect||el===condSelect) savePreferences(); makeLink(false); }));
}
...
// ---------------- Photo + Local OCR + Guided Scan · v85 FOCUSED SCAN ----------------
...
function resetPhoto(){
  ...
  if(scanOpenBtn){ scanOpenBtn.hidden=true; scanOpenBtn.href='#'; }
...
function loadPhotoFile(file, options={}){
  ...
  img.onload = () => {
    ...
    setTimeout(()=>loadVisualFiles().then(()=>{if(!visualIndexReady&&!visualIndexBuilding)buildVisualIndex().catch(()=>{});}),80);
  };
...
function makeOcrCanvas(){
  ...
  // v85 FOCUSED SCAN: het live kader is een richtlijn, niet de echte kaartgrens.
  // Zoek daarom OOK bij camerafoto's eerst de gele kaartgrens. Dit normaliseert de kaart
  // voordat we exact drie kleine zones lezen: naam, collector number en set-symbool.
...
function pokemonCandidates(){
  ...
  const source = (DATA && DATA.knownCards) ? DATA.knownCards : (EMBEDDED_DATA.knownCards || []);
  const known = source.map(c=>c.name);
...
function findPokemonName(text){
  ...
  // v85: tiny top-line OCR often clips exactly one leading/trailing letter
  // (e.g. "irachi" instead of "Jirachi", "emoraid" instead of "Remoraid").
  // Only accept this for names >= 6 chars to avoid short-name false positives.
  const compact=simpleNorm(clean);
  const clipped = candidates
    .filter(n=>simpleNorm(n).length>=6)
    .map(n=>({name:n,nn:simpleNorm(n)}))
    .filter(x=>compact.includes(x.nn.slice(1)) || compact.includes(x.nn.slice(0,-1)))
    .sort((a,b)=>b.nn.length-a.nn.length)[0];
  if(clipped) return {name:clipped.name, confidence:'redelijk', score:.90, safe:true};
...
const SET_TOTAL_HINTS = {
  ...
  95:['EX TEAM MAGMA AQUA'],97:['EX DRAGON'],100:['EX SANDSTORM','EX CRYSTAL GUARDIANS','MAJESTIC DAWN'],
  101:['EX HIDDEN LEGENDS','EX DRAGON FRONTIERS'],102:['BASE'],105:['NEO DESTINY'],106:['EX EMERALD','GREAT ENCOUNTERS','STORMFRONT'],
  ...
  113:['EX DELTA SPECIES'],115:['EX UNSEEN FORCES'],130:['BASE SET 2','DIAMOND PEARL'],132:['GYM HEROES','GYM CHALLENGE','SECRET WONDERS'],
  124:['MYSTERIOUS TREASURES'],144:['SKYRIDGE'],146:['LEGENDS AWAKENED'],147:['AQUAPOLIS'],165:['EXPEDITION'],92:['EX LEGEND MAKER'],12:['EX TRAINER KIT 2'],17:['POP SERIES 1','POP SERIES 2','POP SERIES 3','POP SERIES 4','POP SERIES 5','POP SERIES 6','POP SERIES 7','POP SERIES 8','POP SERIES 9']
};
...
function detectSetSymbolLocal(canvas,candidateCards=[]){
  ...
  if(!best)return null; const margin=best.score-second, threshold=scoped?.54:.74, neededMargin=scoped?.08:.12; if(best.score<threshold||margin<neededMargin)return null;
...
async function scanCollectorEvidence(worker, canvas, guided, candidateCards=[]){
  ...
  // v80 FAST PATH: the two most useful crops + two passes first. Easy cards stop here.
  for(const rr of rects.slice(0,2)){
    for(const pass of quickPasses.slice(0,2)){
      const hit=await runPass(rr,pass);
      if(hit) return hit;
    }
  }
  // Compatibility path: only difficult cards pay for the remaining OCR work.
  for(const rr of rects){
    for(const pass of quickPasses){
      if(rects.indexOf(rr)<2 && quickPasses.indexOf(pass)<2) continue;
      const hit=await runPass(rr,pass);
      if(hit) return hit;
    }
  }
...
async function ensureOcrWorker(quiet=false){
  if(ocrWorker) return ocrWorker;
  if(ocrWorkerPromise){
    if(!quiet) ocrQuietWarmup=false;
    return ocrWorkerPromise;
  }
  ocrQuietWarmup=quiet;
  ocrWorkerPromise=(async()=>{
    const T = await loadTesseract();
    if(!quiet) setOcrProgress(8,'OCR-engine laden… eerste keer kan wat langer duren.');
    const worker = await T.createWorker('eng', 1, {
      logger: m => {
        if(!ocrQuietWarmup && m && m.status === 'recognizing text' && typeof m.progress === 'number'){
  ...
  finally{ ocrWorkerPromise=null; ocrQuietWarmup=false; }
}
function prewarmOcr(){
  if(ocrWorker||ocrWorkerPromise) return;
  ensureOcrWorker(true).catch(()=>{});
}

// --- v86 REGRESSION-LOCK SCAN --------------------------------------------
// Regression rule: never OCR the whole card. One top composite + one bottom composite.
// Hard cards may use ONE small fallback. This preserves v76/v77 reliability without 40 s scans.
function makeFocusedScanCrop(source, fracRect, options={}){
  if(!source) return null;
  const rect={
    left:Math.round(source.width*fracRect.left), top:Math.round(source.height*fracRect.top),
    width:Math.round(source.width*fracRect.width), height:Math.round(source.height*fracRect.height)
  };
  const sx=Math.max(0,rect.left), sy=Math.max(0,rect.top);
  const sw=Math.max(1,Math.min(source.width-sx,rect.width));
  const sh=Math.max(1,Math.min(source.height-sy,rect.height));
  const targetWidth=options.targetWidth||1250;
  const zoom=Math.max(1,Math.min(options.maxZoom||8,targetWidth/sw));
  return makeEnhancedCrop(source,{left:sx,top:sy,width:sw,height:sh},{zoom,contrast:options.contrast||1.35,threshold:options.threshold,invert:!!options.invert});
}
function makeAdaptiveNumberCrop(source, fracRect, targetWidth=1300){
  if(!source) return null;
  const sx=Math.max(0,Math.round(source.width*fracRect.left));
  const sy=Math.max(0,Math.round(source.height*fracRect.top));
  const sw=Math.max(1,Math.min(source.width-sx,Math.round(source.width*fracRect.width)));
  const sh=Math.max(1,Math.min(source.height-sy,Math.round(source.height*fracRect.height)));
  const zoom=Math.max(1,Math.min(9,targetWidth/sw));
  ...
  ctx.drawImage(source,sx,sy,sw,sh,0,0,out.width,out.height);
  const img=ctx.getImageData(0,0,out.width,out.height), d=img.data, w=out.width, h=out.height;
  const gray=new Uint8Array(w*h), integral=new Float64Array((w+1)*(h+1));
  for(let y=0;y<h;y++){
    let rowsum=0;
    for(let x=0;x<w;x++){
      const i=(y*w+x)*4; const g=Math.round(d[i]*.299+d[i+1]*.587+d[i+2]*.114); gray[y*w+x]=g; rowsum+=g;
  ...
  for(let y=0;y<h;y++) for(let x=0;x<w;x++){
    ...
    const A=integral[y0*(w+1)+x0], B=integral[y0*(w+1)+(x1+1)], CC=integral[(y1+1)*(w+1)+x0], D=integral[(y1+1)*(w+1)+(x1+1)];
    const mean=(D-B-CC+A)/((x1-x0+1)*(y1-y0+1)); const v=gray[y*w+x] > mean-C ? 255 : 0; const i=(y*w+x)*4;
    d[i]=d[i+1]=d[i+2]=v; d[i+3]=255;
  }
  ctx.putImageData(img,0,0); return out;
}
function stackScanCrops(crops,gap=16){
  const valid=(crops||[]).filter(Boolean); if(!valid.length)return null;
  const w=Math.max(...valid.map(c=>c.width)), h=valid.reduce((s,c)=>s+c.height,0)+gap*(valid.length-1);
  const out=document.createElement('canvas'); out.width=w; out.height=h; const ctx=out.getContext('2d',{willReadFrequently:true});
  ctx.fillStyle='#fff';ctx.fillRect(0,0,w,h);let y=0;for(const c of valid){ctx.drawImage(c,0,y,c.width,c.height);y+=c.height+gap;}return out;
}
function makeColorScanCrop(source, fracRect, options={}){
  if(!source) return null;
  const sx=Math.max(0,Math.round(source.width*fracRect.left));
  const sy=Math.max(0,Math.round(source.height*fracRect.top));
  const sw=Math.max(1,Math.min(source.width-sx,Math.round(source.width*fracRect.width)));
  const sh=Math.max(1,Math.min(source.height-sy,Math.round(source.height*fracRect.height)));
  const targetWidth=options.targetWidth||1450;
  const zoom=Math.max(1,Math.min(options.maxZoom||7,targetWidth/sw));
  const out=document.createElement('canvas'); out.width=Math.max(1,Math.round(sw*zoom)); out.height=Math.max(1,Math.round(sh*zoom));
  const ctx=out.getContext('2d',{willReadFrequently:true}); ctx.imageSmoothingEnabled=true; ctx.imageSmoothingQuality='high';
  ctx.fillStyle='#fff';ctx.fillRect(0,0,out.width,out.height);ctx.drawImage(source,sx,sy,sw,sh,0,0,out.width,out.height);
  return out;
}
function scanEmbeddedCandidatesByName(name,lang='EN'){
  return (EMBEDDED_DATA.knownCards||[]).filter(c=>c.language===lang&&cardNameMatches(c,name));
}
function uniqueEmbeddedNameCard(name,lang='EN'){
  const hits=scanEmbeddedCandidatesByName(name,lang).filter(c=>c.verified!==false);
  return hits.length===1?hits[0]:null;
}
function stableNameComposite(canvas){
  // Covers WotC, e-Card, EX and DP headers without reading attacks/body text.
  return stackScanCrops([
    makeFocusedScanCrop(canvas,{left:.015,top:.005,width:.78,height:.15},{targetWidth:1200,contrast:1.15,maxZoom:5}),
    makeFocusedScanCrop(canvas,{left:0,top:0,width:.96,height:.20},{targetWidth:1200,contrast:1.25,maxZoom:5}),
    makeFocusedScanCrop(canvas,{left:.015,top:.005,width:.82,height:.17},{targetWidth:1200,contrast:1.70,threshold:170,maxZoom:5})
  ]);
}
function stableNumberComposite(canvas){
  // Context strip + tiny collector corner + adaptive-threshold corner in ONE OCR call.
  return stackScanCrops([
    makeFocusedScanCrop(canvas,{left:.28,top:.755,width:.72,height:.24},{targetWidth:1250,contrast:1.35,maxZoom:6}),
    makeFocusedScanCrop(canvas,{left:.52,top:.82,width:.47,height:.18},{targetWidth:1350,contrast:1.55,maxZoom:8}),
    makeAdaptiveNumberCrop(canvas,{left:.60,top:.84,width:.39,height:.15},1350)
  ],14);
}
function stableNumberFallback(canvas){
  return makeFocusedScanCrop(canvas,{left:.45,top:.79,width:.54,height:.20},{targetWidth:1450,contrast:1.10,maxZoom:8});
}
function embeddedCandidatesByName(name,lang='EN'){
  return (EMBEDDED_DATA.knownCards||[]).filter(c=>c.language===lang && cardNameMatches(c,name));
}
function variantCardFromHeader(name,text,lang='EN'){
  if(!name)return null; const compact=simpleNorm(text); if(!compact)return null;
  const cards=embeddedCandidatesByName(name,lang); const hits=[];
  for(const c of cards){
    for(const alias of (c.aliases||[])){
      const a=simpleNorm(alias); if(a.length<6)continue;
      if(compact.includes(a) || compact.includes(a.slice(1)) || compact.includes(a.slice(0,-1))) hits.push(c);
      else {
        // Variant markers such as Lv.42 / Delta Species are strong when paired with the recognized name.
        const nums=(a.match(/\d+/g)||[]); if(nums.length && nums.every(n=>compact.includes(n))) hits.push(c);
      }
    }
  }
  const uniq=[...new Map(hits.map(c=>[c.key,c])).values()]; return uniq.length===1?uniq[0]:null;
}
function focusedCollectorFromText(text,candidateCards=[]){
  const raw=String(text||''), lines=raw.split(/\r?\n/).map(x=>x.trim()).filter(Boolean), seen=[];
  const add=(x,source)=>{if(!x)return;const key=`${cleanNumber(x.number)}|${Number(x.total)}`;seen.push({...x,key,card:x.candidateCard||null,score:Number(x.score||0),source:source||x.source||'ocr'});};
  for(const line of lines){add(extractCollectorNumberLoose(line,candidateCards),'line');const p=matchPartialCollectorToCandidates(line,candidateCards);if(p)add({number:p.number,total:p.total,candidateCard:p.card,score:p.score},p.reason);}
  add(extractCollectorNumberLoose(raw,candidateCards),'combined'); const m=matchCollectorToCandidates(raw,candidateCards);if(m)add({number:m.number,total:m.total,candidateCard:m.card,score:m.score},m.reason);
  if(!seen.length)return null; const grouped=new Map();
  for(const x of seen){const g=grouped.get(x.key)||{...x,count:0,maxScore:0,card:x.card||null};g.count++;g.maxScore=Math.max(g.maxScore,x.score||0);if(!g.card&&x.card)g.card=x.card;grouped.set(x.key,g);}
  const ranked=[...grouped.values()].sort((a,b)=>((b.card?2:0)+(b.count>=2?1:0)+b.maxScore)-((a.card?2:0)+(a.count>=2?1:0)+a.maxScore));
  const best=ranked[0], supported=Object.prototype.hasOwnProperty.call(SET_TOTAL_HINTS,Number(best.total));
  if(best.card||best.count>=2||(supported&&best.maxScore>=.94))return {number:String(parseInt(best.number,10)),total:Number(best.total),candidateCard:best.card||null,score:best.maxScore,count:best.count,source:best.source};
  return null;
}
function uniqueDataCollector(number,total,lang='EN'){
  const n=cleanNumber(number),t=Number(total),source=(DATA&&DATA.knownCards)||EMBEDDED_DATA.knownCards||[];
  const hits=source.filter(c=>c.language===lang&&cleanNumber(c.number)===n&&setTotalForSet(c.set)===t);return hits.length===1?hits[0]:null;
}
// --- end v85 stable helpers -------------------------------------------------
async function legacyOcrScanCurrentPhoto(){
  if(!photoImage||ocrBusy)return;
  const scanStartedAt=performance.now();ocrBusy=true;scanPhotoBtn.disabled=true;
  if(ocrResult){ocrResult.hidden=true;ocrResult.textContent='';}
  try{
    setOcrProgress(4,'Kaart uitlijnen…'); const canvas=makeOcrCanvas(); if(!canvas)throw new Error('Geen foto geladen'); const worker=await ensureOcrWorker();

    // PASS 1 (v86): read ONLY the actual printed name line. Preserve colour.
    // This fixes pale EX/e-Card headers where grayscale composites lost Cradily/Poochyena/Dunsparce.
    setOcrProgress(16,'Naam lezen…');
    const namePrimary=makeColorScanCrop(canvas,{left:0,top:.005,width:.70,height:.135},{targetWidth:1500,maxZoom:7});
    ...
    let nameScan=await worker.recognize(namePrimary||canvas); let nameText=nameScan?.data?.text||'';
    let trainerGuess=findTrainerCandidate(nameText),trainerDetected=looksLikeTrainer(nameText)||!!trainerGuess,trainerTitle=trainerGuess||(trainerDetected?extractTrainerTitle(nameText):'');
    let found=trainerDetected?null:findPokemonName(nameText);

    // One narrow fallback only when the first name line is missing/uncertain. Still no body/attack OCR.
    if((!found||!found.safe)&&!trainerDetected){
      setOcrProgress(29,'Naam extra controleren…');
      const nameFallback=makeColorScanCrop(canvas,{left:0,top:0,width:.84,height:.18},{targetWidth:1550,maxZoom:7});
      ...
      const r=await worker.recognize(nameFallback||canvas); const t=r?.data?.text||''; nameText+=' '+t;
      const retry=findPokemonName(nameText); if(retry&&(!found||retry.score>found.score))found=retry;
      trainerGuess=findTrainerCandidate(nameText);trainerDetected=looksLikeTrainer(nameText)||!!trainerGuess;trainerTitle=trainerGuess||(trainerDetected?extractTrainerTitle(nameText):'');
    }

    const provisionalName=trainerTitle||found?.name||'';
    let nameCandidates=provisionalName?localCandidatesByName(provisionalName,langSelect.value):[];
    const embeddedNameCandidates=provisionalName?embeddedCandidatesByName(provisionalName,langSelect.value):[];
    const variantCard=provisionalName?variantCardFromHeader(provisionalName,nameText,langSelect.value):null;

    // PASS 2: ONLY THE LOWER CARD STRIP / COLLECTOR CORNER.
    setOcrProgress(48,'Kaartnummer lezen…'); const numberCanvas=stableNumberComposite(canvas);
    await worker.setParameters({tessedit_pageseg_mode:'6',tessedit_char_whitelist:'0123456789/',preserve_interword_spaces:'1'});
    let nr=await worker.recognize(numberCanvas||canvas); let numberText=nr?.data?.text||'';
    let collector=focusedCollectorFromText(numberText,nameCandidates), totalHint=collector?.total||extractLikelySetTotal(numberText);

    // One fallback for difficult silver/foil DP-era bottoms. It stays in the bottom 20% only.
    if(!collector&&!totalHint&&(found||variantCard)){
      setOcrProgress(63,'Nummer extra controleren…'); const fb=stableNumberFallback(canvas);
      await worker.setParameters({tessedit_pageseg_mode:'11',tessedit_char_whitelist:'0123456789/',preserve_interword_spaces:'1'});
      const rr=await worker.recognize(fb||canvas); const ft=rr?.data?.text||''; numberText+=' '+ft;
      collector=focusedCollectorFromText(numberText,nameCandidates); totalHint=collector?.total||extractLikelySetTotal(numberText);
    }

    let collectorCard=collector?.candidateCard||null;
    const stableCollectorCard=collector?uniqueLocalCollector(collector.number,collector.total,langSelect.value):null;
    const globalCollectorCard=collector?(stableCollectorCard||uniqueDataCollector(collector.number,collector.total,langSelect.value)):null;
    if(!collectorCard&&globalCollectorCard)collectorCard=globalCollectorCard;

    // SET: number/database first; symbol is a local tie-breaker, never stronger than a verified number.
    let inferredSet=collectorCard?.set||null;
    if(!inferredSet&&collector)inferredSet=inferSetFromTotal(collector.total,nameCandidates);
    if(!inferredSet&&totalHint)inferredSet=inferSetFromTotal(totalHint,nameCandidates);
    const specialHint=specialScanHint(provisionalName,collector);if(!inferredSet&&specialHint?.set)inferredSet=specialHint.set;
    let symbolEvidence=null;
    if(!trainerDetected&&!inferredSet){symbolEvidence=detectSetSymbolLocal(canvas,nameCandidates);if(symbolEvidence?.set)inferredSet=symbolEvidence.set;}

    let displayName=trainerTitle||found?.name||'',canAutofill=!!trainerTitle||!!found?.safe,localInference=variantCard||collectorCard||globalCollectorCard||null;
    if(variantCard){displayName=variantCard.name;inferredSet=variantCard.set;canAutofill=true;}

    // v86 REGRESSION LOCK: curated scan DB is allowed to confirm a name when exactly one verified
    // classic card with that name is present. Dynamic DP/POP catalog never creates this shortcut.
    // This safely rescues Poochyena/Cradily when their tiny number is unreadable.
    if(displayName&&!localInference){
      const stableNameCard=uniqueEmbeddedNameCard(displayName,langSelect.value);
      if(stableNameCard&&(canAutofill||(found&&found.score>=.84))){
        localInference=stableNameCard; displayName=stableNameCard.name; inferredSet=stableNameCard.set; canAutofill=true;
      }
    }


    // Reconcile against database. OCR is evidence; database is the consistency check.
    if(displayName){
      nameCandidates=localCandidatesByName(displayName,langSelect.value);
      const setCandidates=inferredSet?nameCandidates.filter(c=>c.set===inferredSet):nameCandidates;
      if(collector){
        const exact=setCandidates.filter(c=>cleanNumber(c.number)===cleanNumber(collector.number)&&setTotalForSet(c.set)===Number(collector.total));
        if(exact.length===1){localInference=exact[0];displayName=localInference.name;inferredSet=localInference.set;canAutofill=true;}
      }
      // Name + denominator/set may repair a clipped numerator.
      if(!localInference&&inferredSet&&setCandidates.length===1&&(canAutofill||(found&&found.score>=.72))){localInference=setCandidates[0];displayName=localInference.name;canAutofill=true;}
      // Variant alias (Lv.42 / Delta Species) is stronger than a missing collector number.
      if(!localInference&&variantCard){localInference=variantCard;displayName=variantCard.name;inferredSet=variantCard.set;canAutofill=true;}
    }
    if(!canAutofill&&stableCollectorCard){localInference=stableCollectorCard;displayName=stableCollectorCard.name;inferredSet=stableCollectorCard.set;canAutofill=true;}
    else if(!canAutofill&&globalCollectorCard){localInference=globalCollectorCard;displayName=globalCollectorCard.name;inferredSet=globalCollectorCard.set;canAutofill=true;}

    // If only a set-total was read, use name + that set only when it resolves to one card.
    if(displayName&&!localInference&&totalHint){
      const possibleSets=(SET_TOTAL_HINTS[Number(totalHint)]||[]);
      const hits=nameCandidates.filter(c=>possibleSets.includes(c.set));
      if(hits.length===1){localInference=hits[0];displayName=hits[0].name;inferredSet=hits[0].set;canAutofill=true;}
    }

    // Never keep a collector that conflicts with a verified reconciled local card.
    let collectorRejected=false,rejectedCollectorNumber='';
    if(localInference&&collector&&(cleanNumber(localInference.number)!==cleanNumber(collector.number)||setTotalForSet(localInference.set)!==Number(collector.total))){collectorRejected=true;rejectedCollectorNumber=collector.number;}
    const resolvedNumber=(collector&&!collectorRejected?collector.number:'')||localInference?.number||'';
    if(localInference&&!inferredSet)inferredSet=localInference.set;

    nameInput.value=(displayName&&canAutofill)?displayName:''; numberInput.value=resolvedNumber;
    setSelect.value=(inferredSet&&[...setSelect.options].some(o=>o.value===inferredSet))?inferredSet:'AUTO'; updateCustomSelects();

    const seconds=((performance.now()-scanStartedAt)/1000).toFixed(1);setOcrProgress(100,'Scan klaar. Naam, nummer en set gecontroleerd.');
    if(ocrResult){
      const pieces=[];
      if(trainerDetected){pieces.push('<strong>Type:</strong> Trainerkaart');pieces.push(trainerTitle?`<strong>Naam:</strong> ${escapeHtml(trainerTitle)}`:'<span class="ocrWarn">Trainernaam nog niet zeker.</span>');}
      else if(displayName&&canAutofill)pieces.push(`<strong>Naam:</strong> ${escapeHtml(displayName)} <small>(${localInference?'database bevestigd':found?.confidence||'OCR'})</small>`);
      else if(found)pieces.push(`<strong>Naam:</strong> ${escapeHtml(found.name)} <small>(mogelijk)</small>`);else pieces.push('<span class="ocrWarn">Naam nog niet zeker.</span>');
      if(resolvedNumber){const total=localInference?setTotalForSet(localInference.set):(collector?.total||totalHint);pieces.push(`<strong>Nummer:</strong> ${escapeHtml(resolvedNumber)}${total?'/'+escapeHtml(total):''}${collectorRejected?' <small>(OCR gecorrigeerd)</small>':''}`);}else pieces.push('<span class="ocrWarn">Kaartnummer nog niet zeker.</span>');
      if(inferredSet){let why=localInference?' <small>(database bevestigd)</small>':collector?' <small>(nummer)</small>':symbolEvidence?.set===inferredSet?` <small>(set-symbool · ${Math.round(symbolEvidence.score*100)}%)</small>`:' <small>(set-totaal)</small>';pieces.push(`<strong>Set:</strong> ${escapeHtml(EMBEDDED_DATA.sets[inferredSet]?.label||inferredSet)}${why}`);}else pieces.push('<span class="ocrWarn">Set nog niet zeker.</span>');
      pieces.push(`<span class="ocrEditionOff">Editie: handmatig · ${editionSelect?.value==='1ST'?'<b>1ST EDITION</b>':'<b>Normaal</b>'}.</span>`);
      if(photoAutoCropped)pieces.push('<strong>Foto:</strong> kaartgrens automatisch uitgelijnd');else if(photoGuidedCrop)pieces.push('<strong>Foto:</strong> camerakader gebruikt');else pieces.push('<strong>Foto:</strong> volledige foto gebruikt');
      pieces.push(`<strong>Scantijd:</strong> ${seconds} s`); pieces.push(`<span class="ocrRaw">Naam OCR: ${escapeHtml(normalizeScanText(nameText).slice(0,140))}<br>Nummer OCR: ${escapeHtml(normalizeScanText(numberText).slice(0,160))}</span>`);
      ocrResult.innerHTML=pieces.join('<br>');ocrResult.hidden=false;if(scanEditionQuick)scanEditionQuick.hidden=false;
    }

    if(displayName&&canAutofill&&resolvedNumber&&inferredSet){await makeLink(false);if(scanOpenBtn&&openBtn.href&&!openBtn.classList.contains('disabled')){scanOpenBtn.href=openBtn.href;scanOpenBtn.hidden=false;}setStatus(`Scan klaar in ${seconds} s: ${displayName} #${resolvedNumber} · ${EMBEDDED_DATA.sets[inferredSet]?.label||inferredSet}.`,'ok');}
    else {if(scanOpenBtn)scanOpenBtn.hidden=true;setStatus(`Scan klaar in ${seconds} s. Controleer alleen de ontbrekende gegevens.`,'warn');}
  }catch(err){console.error(err);setOcrProgress(0,'Scan mislukt. Probeer een scherpere foto.');setStatus(`Scan mislukt: ${err.message||err}`,'err');}
  finally{ocrBusy=false;scanPhotoBtn.disabled=false;}
}

// ====================== v90 VISUAL ENGINE ================================
// Primary recognition is visual. OCR is a fallback only while/if visual references are unavailable.
const VISUAL_CACHE_KEY='cardscout.visual.v90.refs';
let visualSeeds=[]; let visualCatalog=[]; let visualRefs=[]; let visualIndexBuilding=false; let visualIndexReady=false;
let visualLoadPromise=null;
function bitHex(bits){let n=0n;for(const b of bits)n=(n<<1n)|BigInt(b?1:0);return n.toString(16).padStart(Math.ceil(bits.length/4),'0');}
function grayData(canvas,w,h){const c=document.createElement('canvas');c.width=w;c.height=h;const x=c.getContext('2d',{willReadFrequently:true});x.drawImage(canvas,0,0,w,h);const d=x.getImageData(0,0,w,h).data,g=new Float64Array(w*h);for(let i=0;i<w*h;i++)g[i]=d[i*4]*.299+d[i*4+1]*.587+d[i*4+2]*.114;return g;}
function pHashVisual(canvas){const N=32,g=grayData(canvas,N,N),co=[];for(let u=0;u<8;u++)for(let v=0;v<8;v++){let sum=0;for(let x=0;x<N;x++){const cx=Math.cos((2*x+1)*u*Math.PI/(2*N));for(let y=0;y<N;y++)sum+=g[y*N+x]*cx*Math.cos((2*y+1)*v*Math.PI/(2*N));}if(u||v)co.push(sum);}const sorted=[...co].sort((a,b)=>a-b),med=sorted[Math.floor(sorted.length/2)];return bitHex(co.map(v=>v>med));}
function dHashVisual(canvas){const w=9,h=8,g=grayData(canvas,w,h),bits=[];for(let y=0;y<h;y++)for(let x=0;x<8;x++)bits.push(g[y*w+x+1]>g[y*w+x]);return bitHex(bits);}
function aHashVisual(canvas){const w=8,h=8,g=grayData(canvas,w,h),mean=g.reduce((a,b)=>a+b,0)/g.length;return bitHex([...g].map(v=>v>mean));}
function chromaVisual(canvas){const w=6,h=8,c=document.createElement('canvas');c.width=w;c.height=h;const x=c.getContext('2d',{willReadFrequently:true});x.drawImage(canvas,0,0,w,h);const d=x.getImageData(0,0,w,h).data,out=[];for(let i=0;i<w*h;i++){const r=d[i*4],g=d[i*4+1],b=d[i*4+2],sm=r+g+b+1;out.push(Math.round(r/sm*255),Math.round(g/sm*255),Math.round(b/sm*255));}return out;}
function visualNormalize(source){const out=document.createElement('canvas');out.width=252;out.height=352;const x=out.getContext('2d',{willReadFrequently:true});x.fillStyle='#fff';x.fillRect(0,0,252,352);x.drawImage(source,0,0,252,352);return out;}
function visualFingerprint(source){const n=visualNormalize(source);return {p:pHashVisual(n),d:dHashVisual(n),a:aHashVisual(n),c:chromaVisual(n)};}
function popCountBig(v){let n=0;while(v){v&=v-1n;n++;}return n;}
function ham(a,b,bits){try{return popCountBig(BigInt('0x'+a)^BigInt('0x'+b))/bits;}catch(_){return 1;}}
function visualDistance(a,b){if(!a||!b)return 1;const pd=ham(a.p,b.p,63),dd=ham(a.d,b.d,64),ad=ham(a.a,b.a,64);let ss=0,n=Math.min(a.c?.length||0,b.c?.length||0);if(!n)return 1;for(let i=0;i<n;i++){const q=(a.c[i]-b.c[i])/255;ss+=q*q;}const cd=Math.min(1,Math.sqrt(ss/n)*3);return .45*pd+.25*dd+.12*ad+.18*cd;}
function cardForVisualKey(key){return (EMBEDDED_DATA.knownCards||[]).find(c=>c.key===key)||(DATA?.knownCards||[]).find(c=>c.key===key)||null;}
async function loadVisualFiles(){if(visualLoadPromise)return visualLoadPromise;visualLoadPromise=(async()=>{try{const [a,b]=await Promise.all([fetch('visual-seeds.json?build=90',{cache:'no-cache'}).then(r=>r.json()),fetch('visual-catalog.json?build=90',{cache:'no-cache'}).then(r=>r.json())]);visualSeeds=a.seeds||[];visualCatalog=b.refs||[];}catch(e){console.warn('Visual files',e);}try{const cache=JSON.parse(localStorage.getItem(VISUAL_CACHE_KEY)||'null');if(cache?.version==='v90'&&Array.isArray(cache.refs))visualRefs=cache.refs;}catch(_){}visualIndexReady=visualRefs.length>=Math.max(1,Math.floor(visualCatalog.length*.9));return true;})();return visualLoadPromise;}
async function imageFromUrlForVisual(url){const r=await fetch(url,{mode:'cors',cache:'force-cache'});if(!r.ok)throw new Error('ref '+r.status);const blob=await r.blob();if('createImageBitmap'in window)return await createImageBitmap(blob);return await new Promise((res,rej)=>{const u=URL.createObjectURL(blob),im=new Image();im.onload=()=>{URL.revokeObjectURL(u);res(im)};im.onerror=()=>{URL.revokeObjectURL(u);rej(new Error('image'))};im.src=u;});}
function saveVisualCache(){try{localStorage.setItem(VISUAL_CACHE_KEY,JSON.stringify({version:'v90',refs:visualRefs}));}catch(e){console.warn('Visual cache full',e);}}
async function buildVisualIndex(){await loadVisualFiles();if(visualIndexBuilding||visualIndexReady||!visualCatalog.length)return;visualIndexBuilding=true;const have=new Set(visualRefs.map(x=>x.key));const priorityKeys=new Set(visualSeeds.map(x=>x.key));const todo=visualCatalog.filter(x=>!have.has(x.key)).sort((a,b)=>(priorityKeys.has(b.key)?1:0)-(priorityKeys.has(a.key)?1:0));let done=0,dirty=0;const workers=Array.from({length:3},async()=>{while(todo.length){const ref=todo.shift();if(!ref)break;try{const im=await imageFromUrlForVisual(ref.url);const fp=visualFingerprint(im);visualRefs.push({key:ref.key,fp});dirty++;}catch(_){}done++;if(dirty>=12){saveVisualCache();dirty=0;}if(done%10===0)await new Promise(r=>setTimeout(r,0));}});await Promise.all(workers);saveVisualCache();visualIndexReady=visualRefs.length>=Math.max(1,Math.floor(visualCatalog.length*.85));visualIndexBuilding=false;}
function groupedVisualMatches(query){const best=new Map();for(const r of [...visualSeeds,...visualRefs]){if(!r?.key||!r?.fp)continue;const d=visualDistance(query,r.fp);const old=best.get(r.key);if(!old||d<old.distance)best.set(r.key,{key:r.key,distance:d,source:visualSeeds.includes(r)?'seed':'official'});}return [...best.values()].sort((a,b)=>a.distance-b.distance);}
function visualConfidence(distance){return Math.max(0,Math.min(99,Math.round((1-distance/.34)*100)));}
function visualStrong(ranked){if(!ranked.length)return false;const a=ranked[0],b=ranked[1];const margin=b?b.distance-a.distance:.2;return a.distance<=.115 || (a.distance<=.18&&margin>=.035) || (a.source==='official'&&a.distance<=.21&&margin>=.045);}
async function applyVisualMatch(match,seconds){const card=cardForVisualKey(match.key);if(!card)return false;nameInput.value=card.name;numberInput.value=cleanNumber(card.number);setSelect.value=[...setSelect.options].some(o=>o.value===card.set)?card.set:'AUTO';updateCustomSelects();await makeLink(false);const total=setTotalForSet(card.set);const pct=visualConfidence(match.distance);setOcrProgress(100,'Visuele scan klaar.');if(ocrResult){ocrResult.innerHTML=`<strong>Visuele match:</strong> ${escapeHtml(card.name)}<br><strong>Nummer:</strong> ${escapeHtml(cleanNumber(card.number))}${total?'/'+total:''}<br><strong>Set:</strong> ${escapeHtml(EMBEDDED_DATA.sets[card.set]?.label||card.set)}<br><strong>Zekerheid:</strong> ${pct}% <small>(${match.source==='seed'?'regressieprofiel':'officiële referentie'})</small><br><span class="ocrEditionOff">Editie: handmatig · ${editionSelect?.value==='1ST'?'<b>1ST EDITION</b>':'<b>Normaal</b>'}.</span><br><strong>Scantijd:</strong> ${seconds} s`;ocrResult.hidden=false;if(scanEditionQuick)scanEditionQuick.hidden=false;}if(scanOpenBtn&&openBtn.href&&!openBtn.classList.contains('disabled')){scanOpenBtn.href=openBtn.href;scanOpenBtn.hidden=false;}setStatus(`${card.name} #${cleanNumber(card.number)} · ${EMBEDDED_DATA.sets[card.set]?.label||card.set} visueel herkend.`,'ok');return true;}
function renderVisualChoices(ranked,seconds){const picks=ranked.slice(0,3).map(m=>{const c=cardForVisualKey(m.key);if(!c)return'';return `<button class="ocrSetPick visualPick" data-visual-key="${escapeHtml(m.key)}">${escapeHtml(c.name)} #${escapeHtml(cleanNumber(c.number))} · ${escapeHtml(EMBEDDED_DATA.sets[c.set]?.label||c.set)} <small>${visualConfidence(m.distance)}%</small></button>`}).join('');if(ocrResult){ocrResult.innerHTML=`<span class="ocrWarn">Geen match sterk genoeg om automatisch te gokken.</span><br><small>Beste visuele kandidaten:</small><div class="ocrSetChoices">${picks}</div><br><strong>Scantijd:</strong> ${seconds} s`;ocrResult.hidden=false;}if(scanOpenBtn)scanOpenBtn.hidden=true;}
async function scanCurrentPhoto(){if(!photoImage||ocrBusy)return;const started=performance.now();ocrBusy=true;scanPhotoBtn.disabled=true;if(ocrResult){ocrResult.hidden=true;ocrResult.textContent='';}try{setOcrProgress(8,'Kaart visueel analyseren…');await loadVisualFiles();const canvas=makeOcrCanvas();if(!canvas)throw new Error('Geen foto geladen');const query=visualFingerprint(canvas);setOcrProgress(52,'Vergelijken met kaarten…');let ranked=groupedVisualMatches(query);const seconds0=()=>((performance.now()-started)/1000).toFixed(1);if(visualStrong(ranked)){await applyVisualMatch(ranked[0],seconds0());return;}
    // If an official index is not ready yet, start it now; never block this scan on all references.
    if(!visualIndexReady&&!visualIndexBuilding)buildVisualIndex().catch(()=>{});
    // A plausible visual top-3 is safer than OCR inventing a card.
    if(ranked.length&&ranked[0].distance<=.285){renderVisualChoices(ranked,seconds0());setOcrProgress(100,'Visuele kandidaten gevonden.');setStatus('Visuele match is nog niet uniek. Kies alleen als de kaart klopt.','warn');return;}
    // Secondary fallback only. Visual recognition remains the primary engine.
    setOcrProgress(70,'Visueel nog onbekend · snelle tekstfallback…');ocrBusy=false;scanPhotoBtn.disabled=false;await legacyOcrScanCurrentPhoto();return;
  }catch(err){console.error(err);setOcrProgress(0,'Visuele scan mislukt.');setStatus(`Scan mislukt: ${err.message||err}`,'err');}finally{ocrBusy=false;scanPhotoBtn.disabled=false;}}
async function pickVisualKey(key){const match=groupedVisualMatches(visualFingerprint(makeOcrCanvas())).find(x=>x.key===key)||{key,distance:.18,source:'manual'};await applyVisualMatch(match,'—');}
// Warm only the lightweight reference fingerprint index. OCR is no longer preloaded.
if('requestIdleCallback'in window)requestIdleCallback(()=>{loadVisualFiles().then(()=>buildVisualIndex()).catch(()=>{});},{timeout:2200});else setTimeout(()=>{loadVisualFiles().then(()=>buildVisualIndex()).catch(()=>{});},1600);
// ==================== end v90 VISUAL ENGINE ==============================

if(scanPhotoBtn) scanPhotoBtn.addEventListener('click',scanCurrentPhoto);

if(ocrResult){
  ocrResult.addEventListener('click', e => {
    const vbtn=e.target.closest('.visualPick');
    if(vbtn){ pickVisualKey(vbtn.dataset.visualKey).catch(console.error); return; }
    ...
    updateCustomSelects();
    makeLink(false).then(()=>{
      if(scanOpenBtn && openBtn.href && !openBtn.classList.contains('disabled')){ scanOpenBtn.href=openBtn.href; scanOpenBtn.hidden=false; }
    });
    setStatus(`Set gekozen: ${EMBEDDED_DATA.sets[set]?.label || set} · ${name} #${number}. Cardmarket staat klaar.`, 'ok');
...
updateCatalogBadges();
scheduleCatalogPack();
setStatus('v86 · Regression Lock · snelle scan + stabiele OCR + DB-rescue.', 'ok');
