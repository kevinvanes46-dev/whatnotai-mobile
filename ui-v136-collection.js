...
(() => {
  ...
  const KEY='cardscout_collection_v133'; // keep same key so all existing v133/v134 collection data survives v135
  ...
  function normalizeItem(x){
    ...
    if(!CONDITIONS.includes(String(x.condition||'').toUpperCase())) x.condition='NM';
    if(!x.variant) x.variant='NORMAL';
    return x;
  }
  ...
  function migrate(){
    ...
    let changed=false;
    for(const x of arr){
      const before=JSON.stringify({condition:x.condition,variant:x.variant});
      normalizeItem(x);
      if(before!==JSON.stringify({condition:x.condition,variant:x.variant})) changed=true;
    }
    if(changed) write(arr);
  }

  function mergeDuplicateAfterIdentityChange(arr, idx){
    const x=arr[idx];
    if(!x) return arr;
    x.id=itemKey(x);
    const other=arr.findIndex((y,j)=>j!==idx && itemKey(y)===x.id);
    if(other>=0){
      arr[other].qty=(Number(arr[other].qty)||1)+(Number(x.qty)||1);
      if(arr[other].paidEach==null && x.paidEach!=null) arr[other].paidEach=x.paidEach;
      arr.splice(idx,1);
    }
    return arr;
  }
  ...
  function render(){
    const arr=read().map(normalizeItem);
    ...
    const market=arr.reduce((sum,x)=>sum+(Number(x.price)||0)*(Number(x.qty)||1),0); marketEl&&(marketEl.textContent=eur(market));
    if(!list)return;
    ...
    list.innerHTML=arr.map((x,i)=>{
      ...
      const source=x.priceSource || 'prijs nog laden';
      return `<article class="collectionCard" data-i="${i}">
        <div class="collectionCardMain"><div><h3>${esc(x.name)}${x.number?' #'+esc(x.number):''}</h3><p>${esc(x.setName||x.set)} · ${esc(x.language)} · ${esc(variant)}</p></div>
        <div class="collectionValue"><b>${shownPrice?eur(shownPrice):'—'}</b><small>${esc(source)}</small></div></div>

        <div class="collectionMetaActions">
          <button class="conditionBtn" data-act="condition" aria-expanded="false">Staat gekocht: <b>${esc(x.condition)}</b></button>
          <div class="conditionPicker" hidden>
            ${CONDITIONS.map(c=>`<button type="button" data-act="set-condition" data-condition="${c}" class="${c===x.condition?'active':''}">${c}</button>`).join('')}
          </div>
        </div>
    ...
    list.querySelectorAll('[data-act]').forEach(btn=>btn.addEventListener('click',()=>{
      ...
      const arr2=read().map(normalizeItem);
      const x=arr2[idx];
      if(!x)return;
      const act=btn.dataset.act;

      if(act==='condition'){
        const picker=card.querySelector('.conditionPicker');
        if(picker){
      ...
      if(act==='set-condition'){
        const c=String(btn.dataset.condition||'').toUpperCase();
        if(CONDITIONS.includes(c)){
          x.condition=c;
          // Condition is part of the collection identity; merge if the same exact
          // card/variant/condition already exists.
          mergeDuplicateAfterIdentityChange(arr2,idx);
          write(arr2);
          render();
          toast(`Staat opgeslagen: ${c}`);
        }
        return;
      }
      ...
      if(act==='paid'){
        const val=prompt('Aankoopprijs per kaart in euro (bijv. 12,50):',x.paidEach??'');
        if(val!==null){
          const n=Number(String(val).replace(',','.'));
          if(Number.isFinite(n)&&n>=0)x.paidEach=n;
        }
      }
      write(arr2); render();
    }));
  ...
  function priceFrom(card,item){
    ...
    if(!cm)return {price:null,reference:null,updated:0,source:''};

    const updated=Date.parse(cm.updated||'')||Date.now();

    if(item.variant==='STAMPED'){
      // EX-era set-logo stamped cards are reverse-holo variants.
      // Cardmarket pricing in TCGdex exposes foil/reverse values separately.
      // Never fall back to the normal-card trend: that caused the €1.76 bug.
      const reverse=firstFinite(
        cm['trend-holo'],
        cm['avg7-holo'],
        cm['avg30-holo'],
        cm['avg-holo'],
      ...
      return {
        ...
        reference:null,
        updated,
        source:reverse!=null?'CM reverse trend':'geen CM reverse prijs'
      };
    }
    ...
    return {
      ...
      reference:null,
      updated,
      source:normal!=null?'CM trend':'geen CM prijs'
    };
  }
  async function refreshPrices(force=false){
    ...
    for(let i=0;i<arr.length;i++){
      ...
      try{const card=await resolveCard(x); if(!card){failed++;continue} const p=priceFrom(card,x); x.price=p.price; x.priceReference=null; x.priceUpdated=p.updated||Date.now(); x.priceSource=p.source; changed++;}
      catch(_){failed++;}
    }
  ...
  exportBtn?.addEventListener('click',()=>{
    const payload={version:135,exportedAt:new Date().toISOString(),collection:read()};
