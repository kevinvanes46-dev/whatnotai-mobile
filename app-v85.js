...
async function scanCurrentPhoto(){
  ...
  try{
    ...
    // PASS 1: ONLY THE CARD NAME/HEADER.
    setOcrProgress(18,'Naam lezen…'); const nameCanvas=stableNameComposite(canvas);
    await worker.setParameters({tessedit_pageseg_mode:'11',tessedit_char_whitelist:'',preserve_interword_spaces:'1'});
    let nameScan=await worker.recognize(nameCanvas||canvas); let nameText=nameScan?.data?.text||'';
    ...
    // One tiny fallback only if the top composite did not identify anything.
    if(!found&&!trainerDetected){
      setOcrProgress(31,'Naam extra controleren…'); const topFallback=makeFocusedScanCrop(canvas,{left:0,top:0,width:.94,height:.20},{targetWidth:1350,contrast:1.05,maxZoom:6});
      await worker.setParameters({tessedit_pageseg_mode:'11',tessedit_char_whitelist:'',preserve_interword_spaces:'1'});
      const r=await worker.recognize(topFallback||canvas); const t=r?.data?.text||''; nameText+=' '+t; found=findPokemonName(nameText);
    }
    ...
    let collectorCard=collector?.candidateCard||null; const globalCollectorCard=collector?uniqueDataCollector(collector.number,collector.total,langSelect.value):null;
    ...
    if(!canAutofill&&globalCollectorCard){localInference=globalCollectorCard;displayName=globalCollectorCard.name;inferredSet=globalCollectorCard.set;canAutofill=true;}
...
updateCatalogBadges();
scheduleCatalogPack();
setStatus('v80 · Mobile Core · snelle scan + auto-fill + Smart Search.', 'ok');
