...
// ---------------- Photo + Local OCR + Guided Scan v70 ----------------
...
function extractLikelySetTotal(text){
  // Set-detectie krijgt voorrang. Ook als de slash wegvalt (bv. OCR: 62111),
  // proberen we alleen bekende, eenduidige settotalen als suffix te herkennen.
  const t=String(text||'').replace(/O/g,'0').replace(/[Il]/g,'1');
  const nums=[...t.matchAll(/\b(\d{2,6})\b/g)].map(m=>m[1]);
  const hits=[];
  for(const token of nums){
    ...
    for(const totalKey of Object.keys(SET_TOTAL_HINTS)){
      if(token.length>String(totalKey).length && token.endsWith(String(totalKey))){
        const prefix=token.slice(0,-String(totalKey).length);
        const num=Number(prefix);
        const total=Number(totalKey);
        if(prefix && Number.isFinite(num) && num>0 && num<=total+20) hits.push(total);
      }
    }
  }
...
function extractCollectorNumberLoose(text){
  const strict=extractCollectorNumber(text);
  if(strict) return strict;
  const t=String(text||'').replace(/O/g,'0').replace(/[Il]/g,'1');
  const tokens=[...t.matchAll(/\b(\d{3,6})\b/g)].map(m=>m[1]);
  ...
  for(const token of tokens){
    for(const total of totals){
      ...
      if(!token.endsWith(ts) || token.length<=ts.length) continue;
      const prefix=token.slice(0,-ts.length);
      const num=Number(prefix);
      if(prefix && Number.isFinite(num) && num>0 && num<=total+20){
        return {number:String(num), total};
      }
    }
  }
  return null;
...
async function scanCollectorEvidence(worker, canvas, guided){
  const rects = guided ? [
    // Heel strak rond de collector number rechtsonder.
    {left:.56, top:.82, width:.43, height:.17},
    {left:.66, top:.86, width:.33, height:.13},
    // Iets breder voor kaarten waar het nummer wat hoger/lager gedrukt staat.
    {left:.42, top:.76, width:.57, height:.23}
  ] : [
    {left:.48, top:.76, width:.51, height:.23},
    {left:.60, top:.82, width:.39, height:.17}
  ];
  const passes=[
    {zoom:4.0,contrast:1.35,psm:'7'},
    {zoom:4.4,contrast:1.8,threshold:175,psm:'7'},
    {zoom:4.6,contrast:2.0,threshold:145,psm:'13'}
  ];
  let combined='';
  let total=null;
  for(const rr of rects){
    ...
    for(const pass of passes){
      ...
      combined += ' ' + text;
      const collector=extractCollectorNumberLoose(text);
      if(collector) return {collector,total:collector.total,text:combined};
      const maybeTotal=extractLikelySetTotal(text);
      if(maybeTotal){ total=maybeTotal; return {collector:null,total,text:combined}; }
    }
  }
  return {collector:null,total:extractLikelySetTotal(combined),text:combined};
}
...
async function scanCurrentPhoto(){
  ...
  try{
    ...
    let collector=null;
    let numberOcrText='';
    const collectorEvidence=await scanCollectorEvidence(worker,canvas,guided);
    ...
    // v70: als de OCR de naam betrouwbaar heeft maar het minuscule kaartnummer mist,
    ...
    if(ocrResult){
      ...
      else if(displayName && canAutofill){
        const setChoices=localCandidatesByName(displayName,langSelect.value).slice(0,4);
...
bind();
renderSaved();
setStatus('v70 · Set First · DB 253.', 'ok');
