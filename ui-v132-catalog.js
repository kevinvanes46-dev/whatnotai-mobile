...
(() => {
  ...
  const SEARCH_BUILD = '132-full-en-jp';
  const TCGDEX_API = 'https://api.tcgdex.net/v2';
  const ONLINE_CACHE_TTL = 14 * 24 * 60 * 60 * 1000;
  const ONLINE_CACHE_PREFIX = 'cardscout_search_catalog_v132_';
  ...
  function appendSuggestionBatch(){
    ...
    for(let i=renderedSuggestionCount;i<end;i++){
      ...
      btn.querySelector('.suggestionMeta').textContent = `${label} · ${lang}${card.rarity ? ' · '+card.rarity : ''}${sourceNote}`;
  ...
  function renderSuggestions(query){
    ...
    summary.innerHTML = `<b>${results.length} ${results.length === 1 ? 'kaart' : 'kaarten'}</b><span>${loading || 'scroll voor alle matches'}</span>`;
    smartSuggestions.appendChild(summary);
