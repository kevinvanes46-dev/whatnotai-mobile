...
(() => {
  ...
  function suggestionScore(card, tokens, fullQuery){
    ...
    const aliasList = (setInfo?.[card.set]?.aliases || []).map(normalize).filter(Boolean);
  ...
  function renderSuggestions(query){
    ...
    const results = catalog
      .map(c => ({card:c, score:suggestionScore(c,tokens,fullQuery)}))
      .filter(x => x.score >= 0)
      .sort((a,b) => {
    ...
    summary.innerHTML = `<b>${results.length} ${results.length === 1 ? 'kaart' : 'kaarten'}</b><span>scroll voor alle matches</span>`;
    ...
    results.forEach(({card}) => {
      ...
      btn.querySelector('.suggestionMeta').textContent = `${label} · ${lang}${card.rarity ? ' · '+card.rarity : ''}`;
      btn.addEventListener('click', () => {
        ...
        quickInput.value = `${card.name} ${card.number} ${label}`;
        if(typeof updateCustomSelects === 'function') updateCustomSelects();
        savePrefs();
  ...
  async function loadCatalog(){
    try{
      const res = await fetch(`cards.json?build=131-search`, {cache:'no-store'});
      if(!res.ok) return;
  ...
  [langSelect,condSelect,editionSelect].filter(Boolean).forEach(el => el.addEventListener('change', () => {
    savePrefs(); syncPrefs();
    if(el === langSelect && quickInput?.value.trim()) renderSuggestions(quickInput.value);
