...
function loadPhotoFile(file){
  ...
  reader.onerror = () => setStatus('Foto kon niet worden gelezen.', 'warn');
  reader.onload = event => {
    const img = new Image();
    img.onerror = () => setStatus('Foto kon niet worden geopend.', 'warn');
    img.onload = () => {
      photoImage = img;
      photoRotation = 0;
      drawPhoto();
      setStatus('Foto geladen. Controleer of naam en nummer leesbaar zijn.', 'ok');
    };
...
bind();
renderSaved();
setStatus('v64 · Foto Beta · DB 243.', 'ok');
renderSaved();
