...
function parseQuick(){
  ...
  // v68: 1st Edition is een echte Cardmarket-offerfilter en mag nooit verdwijnen.
...
// ---------------- Photo + Local OCR + Guided Scan v67 ----------------
...
function loadPhotoFile(file, options={}){
  ...
  if(editionSelect){ editionSelect.value='AUTO'; updateCustomSelects(); }
  ...
  img.onload = () => {
    ...
    drawPhoto();
    if(activePhotoUrl){ URL.revokeObjectURL(activePhotoUrl); activePhotoUrl = null; }
    setStatus('Foto geladen. De volledige foto staat nu gecentreerd in beeld.', 'ok');
  };
...
function extractCollectorNumber(text){
  const t=String(text||'').replace(/O/g,'0').replace(/I/g,'1');
  const frac=[...t.matchAll(/\b(\d{1,3})\s*[\/|]\s*(\d{1,3})\b/g)]
    .map(m=>({number:String(parseInt(m[1],10)), total:parseInt(m[2],10)}))
    .filter(x=>x.total>=5 && x.total<=300 && Number(x.number)<=x.total+20);
  return frac[0] || null;
}
const SET_TOTAL_HINTS = {
  102:['BASE'],64:['JUNGLE'],62:['FOSSIL'],130:['BASE SET 2'],82:['ROCKET'],
  111:['NEO GENESIS'],75:['NEO DISCOVERY'],66:['NEO REVELATION'],113:['NEO DESTINY'],
  110:['LEGENDARY COLLECTION'],165:['EXPEDITION'],186:['AQUAPOLIS'],182:['SKYRIDGE']
};
function inferSetFromTotal(total){
...
async function detectFirstEditionStamp(worker, canvas, guided){
  // Western WOTC stamp sits on the left below/alongside the artwork. We OCR a tight,
  // high-contrast crop. False positives are avoided by requiring the word EDITION.
  const rect=guided
    ? {left:Math.round(canvas.width*.01),top:Math.round(canvas.height*.28),width:Math.round(canvas.width*.36),height:Math.round(canvas.height*.42)}
    : {left:0,top:Math.round(canvas.height*.23),width:Math.round(canvas.width*.42),height:Math.round(canvas.height*.48)};
  const crop=makeEnhancedCrop(canvas,rect,{zoom:3.0,contrast:2.0,threshold:150});
  if(!crop) return {detected:false,text:''};
  ...
  let text=normalizeScanText(r?.data?.text || '');
  let compact=text.toLowerCase().replace(/[^a-z0-9]/g,'');
  let detected=compact.includes('edition') || /editi[o0]n/.test(compact) || /edit[i1l][o0]n/.test(compact);

  // Een tweede pass zonder harde threshold helpt bij glanzende sleeves/toploaders.
  if(!detected){
    const soft=makeEnhancedCrop(canvas,rect,{zoom:3.2,contrast:2.35});
    if(soft){
      await worker.setParameters({tessedit_pageseg_mode:'11',tessedit_char_whitelist:'EDITIONeditionIilL10 '});
      const r2=await worker.recognize(soft);
      const text2=normalizeScanText(r2?.data?.text || '');
      text += ' ' + text2;
      compact=text.toLowerCase().replace(/[^a-z0-9]/g,'');
      detected=compact.includes('edition') || /editi[o0]n/.test(compact) || /edit[i1l][o0]n/.test(compact);
    }
  }
...
function findKnownLocal(name, number, setKey){
  ...
  return list.find(c => (!name || simpleNorm(c.name)===simpleNorm(name)) && (!number || String(parseInt(c.number,10))===String(parseInt(number,10))) && (!setKey || c.set===setKey)) || null;
}
...
async function scanCurrentPhoto(){
  ...
  try{
    ...
    // v68: a second high-contrast pass over a wider top band catches names such as
    ...
    setOcrProgress(72,'Kaartnummer lezen…');
    let collector=null;
    if(guided){
      const numberRect={
        left:Math.round(canvas.width*.55),
        top:Math.round(canvas.height*.82),
        width:Math.round(canvas.width*.44),
        height:Math.round(canvas.height*.17)
      };
      await worker.setParameters({tessedit_pageseg_mode:'7',tessedit_char_whitelist:'0123456789/'});
      const num=await worker.recognize(canvas,{rectangle:numberRect});
      let numText=num?.data?.text || '';
      fullText += ' ' + numText;
      collector=extractCollectorNumber(numText);
      if(!collector){
        const wideNumberRect={left:Math.round(canvas.width*.42),top:Math.round(canvas.height*.72),width:Math.round(canvas.width*.57),height:Math.round(canvas.height*.27)};
        const enhancedNum=makeEnhancedCrop(canvas,wideNumberRect,{zoom:3.0,contrast:2.0,threshold:155});
        if(enhancedNum){
          await worker.setParameters({tessedit_pageseg_mode:'11',tessedit_char_whitelist:'0123456789/'});
          const nr2=await worker.recognize(enhancedNum);
          numText += ' ' + (nr2?.data?.text || '');
          fullText += ' ' + (nr2?.data?.text || '');
          collector=extractCollectorNumber(numText);
        }
      }
    }
    ...
    let inferredSet=collector ? inferSetFromTotal(collector.total) : null;
    ...
    setOcrProgress(88,'1st Edition controleren…');
    const editionScan=await detectFirstEditionStamp(worker,canvas,guided);
    if(editionScan.detected && editionSelect) editionSelect.value='1ST';
    ...
    // v68: als de OCR de naam betrouwbaar heeft maar het minuscule kaartnummer mist,
    // mag de lokale catalogus alleen helpen wanneer die naam precies één EN/JP-record
    // oplevert. Zo herstellen we o.a. Chikorita, Metang en Energy Search zonder te gokken.
    let localInference=null;
    if(displayName && canAutofill){
      let candidates=localCandidatesByName(displayName, langSelect.value);
      if(collector) candidates=candidates.filter(c=>cleanNumber(c.number)===cleanNumber(collector.number));
      if(inferredSet) candidates=candidates.filter(c=>c.set===inferredSet);
      if(candidates.length===1) localInference=candidates[0];
      if(localInference && !collector) numberInput.value=cleanNumber(localInference.number);
      if(localInference && !inferredSet) inferredSet=localInference.set;
    }

    // Never overwrite a field with a low-confidence fuzzy guess.
    if(displayName && canAutofill) nameInput.value=displayName;
    if(collector) numberInput.value=collector.number;
    if(inferredSet && [...setSelect.options].some(o=>o.value===inferredSet)) setSelect.value=inferredSet;
    updateCustomSelects();

    setOcrProgress(100,'Scan klaar. Controleer het resultaat.');
    const resolvedNumber=collector?.number || localInference?.number || '';
    ...
    if(ocrResult){
      ...
      if(collector) pieces.push(`<strong>Nummer:</strong> ${escapeHtml(collector.number)}/${collector.total}`);
      ...
      if(inferredSet) pieces.push(`<strong>Set-hint:</strong> ${escapeHtml(EMBEDDED_DATA.sets[inferredSet]?.label || inferredSet)}`);
      if(editionScan.detected) pieces.push('<strong class="ocrEdition">Editie:</strong> <b>1st Edition ✓</b>');
      else pieces.push('<span class="ocrEditionOff">Editie: geen 1st-stempel bevestigd · tik 1ST EDITION als de kaart er wél één heeft.</span>');
      if(local) pieces.push('<strong>Lokale match:</strong> gevonden in CardScout DB');
      if(guided) pieces.push('<strong>Foto:</strong> guided crop gebruikt');
    ...
    if(displayName && canAutofill){
      setStatus(`Scan: ${displayName}${resolvedNumber?` #${resolvedNumber}`:''}${editionScan.detected?' · 1st Edition':''}. Controleer naam, nummer, set, editie en staat.`, 'ok');
...
bind();
renderSaved();
setStatus('v68 · Smart Scan + 1st Edition · DB 251.', 'ok');
