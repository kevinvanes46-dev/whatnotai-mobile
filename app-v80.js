...
// ---------------- v80 MOBILE CORE: preferences + Smart Search + cached DP/POP catalog ----------------
...
// ---------------- Photo + Local OCR + Guided Scan · v80 MOBILE CORE ----------------
...
async function scanCurrentPhoto(){
  ...
  try{
    ...
    // v80 FAST PATH: if the dedicated name strip is already strong, skip two expensive top OCR passes.
    let headerText=nameText;
    let trainerGuess='';
    let trainerDetected=false;
    let trainerTitle='';
    let found=findPokemonName(nameText);
    if(!found?.safe){
      ...
      headerText=headerScan?.data?.text || '';
      ...
      trainerGuess=findTrainerCandidate(headerText);
      trainerDetected=looksLikeTrainer(headerText) || !!trainerGuess;
      trainerTitle=trainerGuess || (trainerDetected ? extractTrainerTitle(headerText) : '');
      found = trainerDetected ? null : findPokemonName(nameText + ' ' + headerText);
    }
    ...
    if(ocrResult){
      ...
      pieces.push(`<strong>Scantijd:</strong> ${((performance.now()-scanStartedAt)/1000).toFixed(1)} s`);
    ...
    if(displayName && canAutofill){
      // v80 AUTO FILL: a successful scan immediately builds the safe Cardmarket route. No Enter required.
      await makeLink(false);
      if(scanOpenBtn && openBtn.href && !openBtn.classList.contains('disabled')){
        scanOpenBtn.href=openBtn.href; scanOpenBtn.hidden=false;
      }
      setStatus(`Scan klaar in ${((performance.now()-scanStartedAt)/1000).toFixed(1)} s: ${displayName}${resolvedNumber?` #${resolvedNumber}`:''}${inferredSet?` · ${EMBEDDED_DATA.sets[inferredSet]?.label || inferredSet}`:''}. Cardmarket staat klaar.`, 'ok');
      if(ocrResult) setTimeout(()=>ocrResult.scrollIntoView({behavior:'smooth',block:'nearest'}),180);
