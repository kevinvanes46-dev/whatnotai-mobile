...
EXTRA_POKEMON_NAMES.forEach((name, i) => {
  ...
  if(!DATA.pokedex[dex]) DATA.pokedex[dex] = name;
});
let lastBuilt = null;
...
function buildSearchTerm(name, number, set){
  ...
  if(def && def.label) return [name, n, def.label].filter(Boolean).join(' ').trim();
...
function loadPhotoFile(file, options={}){
  ...
  img.onload = () => {
    ...
    photoRotation = 0;
    drawPhoto();
    // Warm OCR in the background after a photo is chosen; scanning remains fully local in-browser.
    setTimeout(()=>ensureOcrWorker().catch(()=>{}),40);
    if(activePhotoUrl){ URL.revokeObjectURL(activePhotoUrl); activePhotoUrl = null; }
...
function pokemonCandidates(){
  const vals = Object.values(DATA.pokedex || {});
  const known = (DATA.knownCards || []).flatMap(c=>[c.name,...(c.aliases||[])]);
...
function findPokemonName(text){
  ...
  // Camera OCR often loses one edge character on metallic/light title bars (e.g. Jirachi -> lirachiw).
  // Accept only a long inner substring (>=5 chars); this is still restricted to the title ROI.
  if(!best){
    for(const name of candidates){
      const nn=simpleNorm(name);
      if(nn.length<6) continue;
      const innerA=nn.slice(1), innerB=nn.slice(0,-1);
      for(const word of words){
        const ww=simpleNorm(word);
        if((innerA.length>=5 && ww.includes(innerA)) || (innerB.length>=5 && ww.includes(innerB))){
          best={name,score:.89};break;
        }
      }
      if(best) break;
    }
...
// ============================================================
// v100 CORE3 SCANNER — ONLY name + collector number + set.
// Camera geometry from v77 is intentionally frozen.
// No attack/body/full-card OCR is allowed in this scanner.
// ============================================================
const CORE3_BUILD = 142;
const CORE3_DEBUG_KEY = 'cardscout_vision_failed_scans';
try{if(!localStorage.getItem(CORE3_DEBUG_KEY)){const old=localStorage.getItem('cardscout_vision_failed_scans_v130');if(old)localStorage.setItem(CORE3_DEBUG_KEY,old);}}catch(_){}
let core3LastScan = null;

function core3FitWidth(source,maxWidth=1450){
  if(!source || source.width<=maxWidth) return source;
  const scale=maxWidth/source.width;
  const out=document.createElement('canvas');out.width=maxWidth;out.height=Math.max(1,Math.round(source.height*scale));
  const ctx=out.getContext('2d');ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';ctx.drawImage(source,0,0,out.width,out.height);return out;
}
function core3StackCanvases(items,pad=18){
  items=(items||[]).filter(Boolean).map(x=>core3FitWidth(x));
  if(!items.length)return null;
  const width=Math.max(...items.map(x=>x.width)),height=items.reduce((s,x)=>s+x.height+pad*2,0);
  const out=document.createElement('canvas');out.width=width;out.height=height;const ctx=out.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,width,height);
  let y=0;for(const item of items){const x=Math.round((width-item.width)/2);y+=pad;ctx.drawImage(item,x,y);y+=item.height+pad;}return out;
}
function core3FracRect(canvas,x,y,w,h){return {left:Math.round(canvas.width*x),top:Math.round(canvas.height*y),width:Math.max(1,Math.round(canvas.width*w)),height:Math.max(1,Math.round(canvas.height*h))};}
function core3NameComposite(canvas,guided){
  // Exact spatial coverage from the last stable v77 camera pipeline, compressed into ONE OCR call.
  // All variants stay inside the top 24% of the image: no attacks/body text.
  const nameRect=guided?core3FracRect(canvas,.025,.02,.73,.14):core3FracRect(canvas,0,0,1,.23);
  const headerRect=guided?core3FracRect(canvas,.02,.01,.96,.22):core3FracRect(canvas,0,0,1,.27);
  const topBand=core3FracRect(canvas,0,0,1,guided?.24:.28);
  return core3StackCanvases([
    makeEnhancedCrop(canvas,nameRect,{zoom:2.2,contrast:1.10}),
    makeEnhancedCrop(canvas,headerRect,{zoom:2.0,contrast:1.00}),
    makeEnhancedCrop(canvas,topBand,{zoom:1.9,contrast:1.45,threshold:150}),
    makeEnhancedCrop(canvas,topBand,{zoom:1.9,contrast:1.85})
  ],14);
}
function core3NumberComposite(canvas,guided){
  // Exact broad lower-edge coverage from v77, but batched into ONE OCR call.
  // The highest crop starts at 74% card height; attacks/body text above that are never OCR'd.
  const rr=guided?[
    [.69,.82,.30,.17],[.54,.78,.45,.21],[.36,.74,.63,.25],[.18,.82,.81,.17]
  ]:[
    [.66,.80,.33,.19],[.50,.74,.49,.25],[.22,.80,.77,.19]
  ];
  const pieces=[];
  for(const [x,y,w,h] of rr){
    const r=core3FracRect(canvas,x,y,w,h);
    pieces.push(makeEnhancedCrop(canvas,r,{zoom:4.0,contrast:1.35}));
    pieces.push(makeEnhancedCrop(canvas,r,{zoom:4.0,contrast:2.05,threshold:165}));
  }
  return core3StackCanvases(pieces,10);
}
function core3ConsensusName(raw){
  // Fallback for Pokemon beyond the small embedded Pokedex: only repeated title-ROI words are eligible.
  const banned=new Set(['basic','pokemon','stage','evolves','from','hp','level','trainer','lv']);
  const words=normalizeScanText(raw).split(/\s+/).filter(Boolean);
  const counts=new Map(),original=new Map();
  for(const w of words){
    const n=simpleNorm(w);if(n.length<4||n.length>18||banned.has(n)||/^\d+$/.test(n))continue;
    counts.set(n,(counts.get(n)||0)+1);if(!original.has(n))original.set(n,w.replace(/[^A-Za-z'-.]/g,''));
  }
  const ranked=[...counts.entries()].filter(([,c])=>c>=2).sort((a,b)=>b[1]-a[1]||b[0].length-a[0].length);
  if(!ranked.length)return '';
  const v=original.get(ranked[0][0])||'';
  return v? v.charAt(0).toUpperCase()+v.slice(1).toLowerCase():'';
}
function core3ExactCard(name,number,total,lang='EN'){
  if(!name||!number||!total)return null;
  const hits=(DATA.knownCards||[]).filter(c=>
    c.language===lang && cardNameMatches(c,name) && cleanNumber(c.number)===cleanNumber(number) && setTotalForSet(c.set)===Number(total)
  );
  return hits.length===1?hits[0]:null;
}
function core3UniqueNameTotal(name,total,lang='EN'){
  if(!name||!total)return null;
  const hits=localCandidatesByName(name,lang).filter(c=>setTotalForSet(c.set)===Number(total));
  return hits.length===1?hits[0]:null;
}
function core3Resolve(nameText,numberText,lang='EN'){
  const found=findPokemonName(nameText);
  const consensus=!found?core3ConsensusName(nameText):'';
  const provisional=found?.name||consensus||'';
  const candidates=provisional?localCandidatesByName(provisional,lang):[];
  let collector=extractCollectorNumberLoose(numberText,candidates);
  let total=collector?.total||extractLikelySetTotal(numberText)||null;
  let card=null;
  let set=null;
  let reason=[];

  if(collector && provisional){
    card=core3ExactCard(provisional,collector.number,collector.total,lang);
    if(card) reason.push('name+collector exact');
  }
  if(!card && collector?.candidateCard){card=collector.candidateCard;reason.push('collector candidate');}
  if(!card && collector){
    const global=uniqueLocalCollector(collector.number,collector.total,lang);
    if(global){card=global;reason.push('collector unique');}
  }
  if(!card && provisional && total){
    const nt=core3UniqueNameTotal(provisional,total,lang);
    if(nt){card=nt;reason.push('name+set-total unique');}
  }
  if(card){
    set=card.set;
    collector={number:cleanNumber(card.number),total:setTotalForSet(card.set),source:reason.join('+'),candidateCard:card};
  }else if(total){
    set=inferSetFromTotal(total,candidates);
    if(set) reason.push('set-total');
  }

  // A name-only match is shown but is NEVER allowed to invent the collector/set.
  // This avoids identifying the wrong printing when the local DB is incomplete.
  const nameSafe=!!((found && (found.safe || found.score>=.84)) || consensus);
  return {found,name:nameSafe?provisional:'',collector,total,set,card,reason:reason.join(', '),nameRaw:nameText,numberRaw:numberText};
}
function core3Thumbnail(canvas){
  try{
    const maxW=260, scale=Math.min(1,maxW/canvas.width), out=document.createElement('canvas');
    out.width=Math.max(1,Math.round(canvas.width*scale));out.height=Math.max(1,Math.round(canvas.height*scale));
    out.getContext('2d').drawImage(canvas,0,0,out.width,out.height);
    return out.toDataURL('image/jpeg',.48);
  }catch(_){return '';}
}
function core3ReadDebug(){
  try{return JSON.parse(localStorage.getItem(CORE3_DEBUG_KEY)||'[]')||[];}catch(_){return [];}
}
function core3WriteDebug(list){
  try{localStorage.setItem(CORE3_DEBUG_KEY,JSON.stringify((list||[]).slice(-40)));}catch(e){console.warn('debug log full',e);}
  core3RenderDebug();
}
function core3LogFailure(reason,scan,canvas,manual=false){
  const list=core3ReadDebug();
  const entry={
    timestamp:new Date().toISOString(),build:CORE3_BUILD,reason,manual,
    source:photoGuidedCrop?'camera-guide':(photoAutoCropped?'gallery-auto':'gallery-full'),
    scanMs:scan?.scanMs||null,
    name:scan?.name||'',number:scan?.collector?.number||'',total:scan?.collector?.total||scan?.total||null,set:scan?.set||'',
    nameOcr:normalizeScanText(scan?.nameRaw||'').slice(0,180),
    numberOcr:normalizeCollectorOcr(scan?.numberRaw||'').replace(/[^0-9/| ._-]/g,'').slice(0,180),
    reasonDetail:scan?.reason||'',
    scanner:'VISION CORE v142 RECOVERY',ocrCalls:scan?.ocrCalls??null,timings:scan?.timings||null,
    visionMeta:scan?.visionMeta||visionLastMeta||null,visionTop3:scan?.visionTop3||[],visionReferenceCount:visionRefs.length,
    currentFields:{name:nameInput?.value||'',number:numberInput?.value||'',set:setSelect?.value||'AUTO'},
    thumbnail:canvas?core3Thumbnail(canvas):''
  };
  list.push(entry);core3WriteDebug(list);return entry;
}
function core3RenderDebug(){
  const count=document.getElementById('debugFailCount');
  if(count) count.textContent=String(core3ReadDebug().length);
}
function core3DownloadDebug(){
  const list=core3ReadDebug();
  const blob=new Blob([JSON.stringify({app:'CardScout',build:CORE3_BUILD,exportedAt:new Date().toISOString(),failedScans:list},null,2)],{type:'application/json'});
  const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`cardscout-failed-scans-${new Date().toISOString().slice(0,10)}.json`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),1200);
}
function core3ClearDebug(){core3WriteDebug([]);}

// ===================== v130 VISION CORE ================================
// Commercial-style architecture: normalize the physical card first, then visual match.
// OCR is a fallback / tie-breaker, never the primary set detector.
const VISION_CACHE_KEY='cardscout.visual.legacy-official-v130.refs';
let visionSeeds=[],visionCatalog=[],visionRefs=[],visionLoadPromise=null,visionIndexBuilding=false,visionIndexReady=false;
let visionLastMeta=null;
function vBitHex(bits){let n=0n;for(const b of bits)n=(n<<1n)|BigInt(b?1:0);return n.toString(16).padStart(Math.ceil(bits.length/4),'0');}
function vGrayData(canvas,w,h){const c=document.createElement('canvas');c.width=w;c.height=h;const x=c.getContext('2d',{willReadFrequently:true});x.drawImage(canvas,0,0,w,h);const d=x.getImageData(0,0,w,h).data,g=new Float64Array(w*h);for(let i=0;i<w*h;i++)g[i]=d[i*4]*.299+d[i*4+1]*.587+d[i*4+2]*.114;return g;}
function vPHash(canvas){const N=32,g=vGrayData(canvas,N,N),co=[];for(let u=0;u<8;u++)for(let v=0;v<8;v++){let sum=0;for(let x=0;x<N;x++){const cx=Math.cos((2*x+1)*u*Math.PI/(2*N));for(let y=0;y<N;y++)sum+=g[y*N+x]*cx*Math.cos((2*y+1)*v*Math.PI/(2*N));}if(u||v)co.push(sum);}const s=[...co].sort((a,b)=>a-b),m=s[Math.floor(s.length/2)];return vBitHex(co.map(q=>q>m));}
function vDHash(canvas){const w=9,h=8,g=vGrayData(canvas,w,h),bits=[];for(let y=0;y<h;y++)for(let x=0;x<8;x++)bits.push(g[y*w+x+1]>g[y*w+x]);return vBitHex(bits);}
function vAHash(canvas){const g=vGrayData(canvas,8,8),m=g.reduce((a,b)=>a+b,0)/g.length;return vBitHex([...g].map(q=>q>m));}
function vChroma(canvas){const c=document.createElement('canvas');c.width=6;c.height=8;const x=c.getContext('2d',{willReadFrequently:true});x.drawImage(canvas,0,0,6,8);const d=x.getImageData(0,0,6,8).data,o=[];for(let i=0;i<48;i++){const r=d[i*4],g=d[i*4+1],b=d[i*4+2],s=r+g+b+1;o.push(Math.round(r/s*255),Math.round(g/s*255),Math.round(b/s*255));}return o;}
function vFingerprint(source){const n=document.createElement('canvas');n.width=252;n.height=352;const x=n.getContext('2d',{willReadFrequently:true});x.fillStyle='#fff';x.fillRect(0,0,n.width,n.height);x.drawImage(source,0,0,n.width,n.height);return {p:vPHash(n),d:vDHash(n),a:vAHash(n),c:vChroma(n)};}
function vPop(v){let n=0;while(v){v&=v-1n;n++;}return n;}
function vHam(a,b,bits){try{return vPop(BigInt('0x'+a)^BigInt('0x'+b))/bits;}catch(_){return 1;}}
function vDistance(a,b){if(!a||!b)return 1;const pd=vHam(a.p,b.p,63),dd=vHam(a.d,b.d,64),ad=vHam(a.a,b.a,64);let ss=0,n=Math.min(a.c?.length||0,b.c?.length||0);if(!n)return 1;for(let i=0;i<n;i++){const q=(a.c[i]-b.c[i])/255;ss+=q*q;}const cd=Math.min(1,Math.sqrt(ss/n)*3);return .45*pd+.25*dd+.12*ad+.18*cd;}
function vCardForKey(key){return (DATA?.knownCards||[]).find(c=>c.key===key)||null;}
function vOrderQuad(q){const p=[...q];const sum=p.map(a=>a.x+a.y),diff=p.map(a=>a.x-a.y);return [p[sum.indexOf(Math.min(...sum))],p[diff.indexOf(Math.max(...diff))],p[sum.indexOf(Math.max(...sum))],p[diff.indexOf(Math.min(...diff))]];}
function vQuadArea(q){let a=0;for(let i=0;i<4;i++){const p=q[i],n=q[(i+1)%4];a+=p.x*n.y-n.x*p.y;}return Math.abs(a)/2;}
function vQuadValid(q,w,h){if(!q||q.length!==4)return false;const z=vOrderQuad(q),top=Math.hypot(z[1].x-z[0].x,z[1].y-z[0].y),bot=Math.hypot(z[2].x-z[3].x,z[2].y-z[3].y),left=Math.hypot(z[3].x-z[0].x,z[3].y-z[0].y),right=Math.hypot(z[2].x-z[1].x,z[2].y-z[1].y),ww=(top+bot)/2,hh=(left+right)/2,r=ww/Math.max(1,hh),area=vQuadArea(z);return area>w*h*.08&&area<w*h*.96&&r>.48&&r<.94&&hh>h*.30&&ww>w*.20;}
function vDetectYellowQuad(source){
 if(!source||source.width<120||source.height<160)return null;
 const maxDim=480,sc=Math.min(1,maxDim/Math.max(source.width,source.height)),w=Math.max(1,Math.round(source.width*sc)),h=Math.max(1,Math.round(source.height*sc));
 const c=document.createElement('canvas');c.width=w;c.height=h;const x=c.getContext('2d',{willReadFrequently:true});x.drawImage(source,0,0,w,h);const d=x.getImageData(0,0,w,h).data,mask=new Uint8Array(w*h);
 let count=0;for(let i=0;i<w*h;i++){const r=d[i*4],g=d[i*4+1],b=d[i*4+2];if(r>128&&g>78&&b<170&&(r-b)>42&&(g-b)>18&&r>g*.80){mask[i]=1;count++;}}
 if(count<w*h*.006)return null;
 // 5x5 dilation joins glare-broken pieces of the outer yellow frame.
 const dil=new Uint8Array(mask.length);for(let yy=2;yy<h-2;yy++)for(let xx=2;xx<w-2;xx++){let on=0;for(let dy=-2;dy<=2&&!on;dy++)for(let dx=-2;dx<=2;dx++)if(mask[(yy+dy)*w+xx+dx]){on=1;break;}if(on)dil[yy*w+xx]=1;}
 const seen=new Uint8Array(dil.length),queue=new Int32Array(dil.length);let best=null;
 for(let sy=0;sy<h;sy++)for(let sx=0;sx<w;sx++){const start=sy*w+sx;if(!dil[start]||seen[start])continue;let qh=0,qt=0;queue[qt++]=start;seen[start]=1;let area=0,minX=sx,maxX=sx,minY=sy,maxY=sy,tl=null,tr=null,br=null,bl=null;
  while(qh<qt){const idx=queue[qh++],py=Math.floor(idx/w),px=idx-py*w;area++;if(px<minX)minX=px;if(px>maxX)maxX=px;if(py<minY)minY=py;if(py>maxY)maxY=py;const s=px+py,df=px-py;if(!tl||s<tl.s)tl={x:px,y:py,s};if(!br||s>br.s)br={x:px,y:py,s};if(!tr||df>tr.d)tr={x:px,y:py,d:df};if(!bl||df<bl.d)bl={x:px,y:py,d:df};for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++){if(!dx&&!dy)continue;const nx=px+dx,ny=py+dy;if(nx<0||ny<0||nx>=w||ny>=h)continue;const ni=ny*w+nx;if(dil[ni]&&!seen[ni]){seen[ni]=1;queue[qt++]=ni;}}}
  const bw=maxX-minX+1,bh=maxY-minY+1,r=bw/Math.max(1,bh);if(area<w*h*.006||bw<w*.20||bh<h*.30||r<.46||r>.98)continue;const cx=(minX+maxX)/2,cy=(minY+maxY)/2,center=Math.hypot((cx-w/2)/w,(cy-h/2)/h),score=area*(1.3-Math.min(.75,center))*(1.1-Math.min(.5,Math.abs(r-.716)));if(!best||score>best.score)best={score,quad:[tl,tr,br,bl]};
 }
 if(!best)return null;let q=best.quad.map(p=>({x:p.x/sc,y:p.y/sc}));q=vOrderQuad(q);if(!vQuadValid(q,source.width,source.height))return null;return {quad:q,method:'yellow-quad',confidence:'high'};
}
function vSolve8(A,b){for(let i=0;i<8;i++){let m=i;for(let r=i+1;r<8;r++)if(Math.abs(A[r][i])>Math.abs(A[m][i]))m=r;[A[i],A[m]]=[A[m],A[i]];[b[i],b[m]]=[b[m],b[i]];let p=A[i][i];if(Math.abs(p)<1e-9)return null;for(let j=i;j<8;j++)A[i][j]/=p;b[i]/=p;for(let r=0;r<8;r++){if(r===i)continue;const f=A[r][i];for(let j=i;j<8;j++)A[r][j]-=f*A[i][j];b[r]-=f*b[i];}}return b;}
function vHomography(dst,src){const A=[],b=[];for(let i=0;i<4;i++){const x=dst[i].x,y=dst[i].y,u=src[i].x,v=src[i].y;A.push([x,y,1,0,0,0,-u*x,-u*y]);b.push(u);A.push([0,0,0,x,y,1,-v*x,-v*y]);b.push(v);}const h=vSolve8(A,b);return h?[h[0],h[1],h[2],h[3],h[4],h[5],h[6],h[7],1]:null;}
function vWarp(source,quad){const W=252,H=352,q=vOrderQuad(quad),dst=[{x:0,y:0},{x:W-1,y:0},{x:W-1,y:H-1},{x:0,y:H-1}],H9=vHomography(dst,q);if(!H9)return null;const sctx=source.getContext('2d',{willReadFrequently:true}),sw=source.width,sh=source.height,sd=sctx.getImageData(0,0,sw,sh).data,out=document.createElement('canvas');out.width=W;out.height=H;const ox=out.getContext('2d',{willReadFrequently:true}),im=ox.createImageData(W,H),od=im.data;
 for(let y=0;y<H;y++)for(let x=0;x<W;x++){const den=H9[6]*x+H9[7]*y+1,sx=(H9[0]*x+H9[1]*y+H9[2])/den,sy=(H9[3]*x+H9[4]*y+H9[5])/den,ix=Math.max(0,Math.min(sw-1,Math.round(sx))),iy=Math.max(0,Math.min(sh-1,Math.round(sy))),si=(iy*sw+ix)*4,di=(y*W+x)*4;od[di]=sd[si];od[di+1]=sd[si+1];od[di+2]=sd[si+2];od[di+3]=255;}ox.putImageData(im,0,0);return out;}
function vVisionCanvas(){const base=makeOcrCanvas();if(!base)return null;const det=vDetectYellowQuad(base);let normalized=base;if(det?.quad){const w=vWarp(base,det.quad);if(w)normalized=w;}else{const b=detectYellowCardBounds(base);if(b)normalized=cropCanvasToBounds(base,b);}visionLastMeta={method:det?.method||(normalized!==base?'yellow-bounds':'guide/fallback'),confidence:det?.confidence||(normalized!==base?'medium':'fallback'),quad:det?.quad||null};return normalized;}
async function vLoadFiles(){
  if(visionLoadPromise)return visionLoadPromise;
  visionLoadPromise=(async()=>{
    try{
      const [a,b]=await Promise.all([
        fetch('visual-seeds.json?build=142',{cache:'no-cache'}).then(r=>r.json()),
        fetch('visual-catalog.json?build=142',{cache:'no-cache'}).then(r=>r.json())
      ]);
      visionSeeds=a.seeds||[];visionCatalog=b.refs||[];
    }catch(e){console.warn('vision files',e);}
    try{
      if(window.CardScoutMemory){
        await CardScoutMemory.init();
        visionRefs=await CardScoutMemory.loadVisionCache('legacy-official-v130');
      }
      if(!visionRefs.length){
        const old=JSON.parse(localStorage.getItem('cardscout.visual.v130.refs')||'null');
        if(Array.isArray(old?.refs)){visionRefs=old.refs; if(window.CardScoutMemory)CardScoutMemory.saveVisionCache('legacy-official-v130',visionRefs).catch(()=>{});}
      }
    }catch(e){console.warn('vision cache restore',e);}
    visionIndexReady=vOfficialCoverage()>=vMinOfficialCoverage();
  })();
  return visionLoadPromise;
}
function vOfficialCoverage(){const wanted=new Set(visionCatalog.map(x=>x.key));return new Set(visionRefs.filter(x=>wanted.has(x.key)).map(x=>x.key)).size;}
function vMinOfficialCoverage(){const n=visionCatalog.length;return Math.max(1,Math.min(n,Math.ceil(n*.95)));}
async function vImage(url){const r=await fetch(url,{mode:'cors',cache:'force-cache'});if(!r.ok)throw new Error('ref '+r.status);const b=await r.blob();if('createImageBitmap'in window)return createImageBitmap(b);return await new Promise((res,rej)=>{const u=URL.createObjectURL(b),im=new Image();im.onload=()=>{URL.revokeObjectURL(u);res(im)};im.onerror=()=>{URL.revokeObjectURL(u);rej(new Error('image'))};im.src=u;});}
function vSaveCache(){
  try{
    if(window.CardScoutMemory){CardScoutMemory.saveVisionCache('legacy-official-v130',visionRefs).catch(e=>console.warn('vision IDB cache',e));return;}
    localStorage.setItem(VISION_CACHE_KEY,JSON.stringify({version:'v140',refs:visionRefs}));
  }catch(e){console.warn('vision cache',e);}
}
async function vBuildIndex(){
  await vLoadFiles();
  if(visionIndexBuilding||!visionCatalog.length)return;
  visionIndexBuilding=true;
  const have=new Set(visionRefs.map(x=>x.key));
  const todo=visionCatalog.filter(x=>!have.has(x.key));
  const total=todo.length; let done=0,dirty=0;
  const workers=Array.from({length:4},async()=>{
    while(todo.length){
      const r=todo.shift();if(!r)break;
      try{const im=await vImage(r.url);visionRefs.push({key:r.key,fp:vFingerprint(im)});dirty++;}catch(_){}
      done++;
      if(dirty>=16){vSaveCache();dirty=0;}
      if(done%10===0){
        const pct=total?Math.round(done/total*100):100;
        if(ocrProgress && !ocrProgress.hidden)setOcrProgress(Math.min(40,10+pct*.3),`Scannerdatabase voorbereiden… ${pct}%`);
        await new Promise(z=>setTimeout(z,0));
      }
    }
  });
  await Promise.all(workers);vSaveCache();
  visionIndexReady=vOfficialCoverage()>=vMinOfficialCoverage();visionIndexBuilding=false;
}
function vMedian(values){if(!values.length)return Infinity;const a=[...values].sort((x,y)=>x-y),m=Math.floor(a.length/2);return a.length%2?a[m]:(a[m-1]+a[m])/2;}
function vRank(fp){
  // Official catalog gets one fair score per identity. Repeated hand-made seeds never get extra lottery tickets.
  const best=new Map();
  for(const r of visionRefs){if(!r?.key||!r?.fp)continue;const d=vDistance(fp,r.fp),old=best.get(r.key);if(!old||d<old.distance)best.set(r.key,{key:r.key,distance:d,source:'official'});}
  const groups=new Map();
  for(const r of visionSeeds){if(!r?.key||!r?.fp||!vCardForKey(r.key))continue;const arr=groups.get(r.key)||[];arr.push(vDistance(fp,r.fp));groups.set(r.key,arr);}
  for(const [key,distances] of groups){
    if(best.has(key))continue; // seeds are fallback only if no official reference exists
    best.set(key,{key,distance:vMedian(distances),source:'seed-fallback',seedCount:distances.length});
  }
  return [...best.values()].sort((a,b)=>a.distance-b.distance);
}
function vSamePrintedCard(a,b){
  if(!a||!b)return false;
  const ca=vCardForKey(a.key),cb=vCardForKey(b.key);
  if(!ca||!cb)return a.key===b.key;
  return String(ca.language||'EN').toUpperCase()===String(cb.language||'EN').toUpperCase()&&ca.set===cb.set&&cleanNumber(ca.number)===cleanNumber(cb.number);
}
function vStrong(rank){
  if(!visionIndexReady||!rank.length)return false;
  const a=rank[0],b=rank.slice(1).find(x=>!vSamePrintedCard(a,x)),margin=b?b.distance-a.distance:.2;
  // Recovery rule: only official references, high catalog coverage, and a clear gap to a genuinely different printing.
  if(a.source!=='official')return false;
  return a.distance<=.18||(a.distance<=.22&&margin>=.035)||(a.distance<=.25&&margin>=.05);
}
function vTopMeta(rank){return rank.slice(0,3).map(x=>{const c=vCardForKey(x.key);return {key:x.key,name:c?.name||'',number:c?.number||'',set:c?.set||'',distance:Number(x.distance.toFixed(4)),source:x.source};});}
async function vApplyCard(card,scan,started,canvas,reason){if(!card)return false;scan=scan||{};scan.card=card;scan.rejected=false;scan.name=card.name;scan.set=card.set;scan.collector={number:cleanNumber(card.number),total:setTotalForSet(card.set),source:reason,candidateCard:card};scan.reason=reason;scan.scanMs=Math.round(performance.now()-started);nameInput.value=card.name;numberInput.value=cleanNumber(card.number);setSelect.value=[...setSelect.options].some(o=>o.value===card.set)?card.set:'AUTO';updateCustomSelects();setOcrProgress(100,'Kaart gevonden.');if(ocrResult){ocrResult.innerHTML=`<strong>Naam:</strong> ${escapeHtml(card.name)}<br><strong>Kaartnummer:</strong> ${escapeHtml(cleanNumber(card.number))}/${escapeHtml(String(setTotalForSet(card.set)||''))}<br><strong>Set:</strong> ${escapeHtml(DATA.sets[card.set]?.label||card.set)}<br><span class="core3Time">Scantijd: ${(scan.scanMs/1000).toFixed(1)} s · ${escapeHtml(reason)}</span><br><button type="button" class="debugMarkBad" id="debugMarkBad">⚠️ Foute scan opslaan</button>`;ocrResult.hidden=false;}core3LastScan={scan,canvas};const mark=document.getElementById('debugMarkBad');if(mark)mark.addEventListener('click',()=>{scan.rejected=true;try{window.CardScoutLearningHooks?.markRejected?.();}catch(_){}core3LogFailure('handmatig gemarkeerd als fout',scan,canvas,true);mark.textContent='✓ Opgeslagen in foutlog';mark.disabled=true;});setStatus(`Scan: ${card.name} #${cleanNumber(card.number)} · ${DATA.sets[card.set]?.label||card.set}.`,'ok');try{await makeLink(false);}catch(_){}if(scanEditionQuick)scanEditionQuick.hidden=false;return true;}
function vUniqueNameCard(name,lang='EN'){if(!name)return null;const hits=localCandidatesByName(name,lang);return hits.length===1?hits[0]:null;}
// Warm references silently; current scan never waits for the full catalog.
if('requestIdleCallback'in window)requestIdleCallback(()=>{vLoadFiles().then(()=>vBuildIndex()).catch(()=>{});},{timeout:700});else setTimeout(()=>{vLoadFiles().then(()=>vBuildIndex()).catch(()=>{});},500);
// =================== end v130 VISION CORE ===============================
...
async function scanCurrentPhoto(){
  ...
  ocrBusy=true;scanPhotoBtn.disabled=true;if(ocrResult){ocrResult.hidden=true;ocrResult.textContent='';}
  const started=performance.now();let canvas=null,scan={name:'',collector:null,total:null,set:'',card:null,reason:'',nameRaw:'',numberRaw:'',scanMs:0};let ranked=[];let ocrCalls=0;const timings={};
  try{
    const t0=performance.now();setOcrProgress(5,'Kaart uitlijnen…');canvas=vVisionCanvas();if(!canvas)throw new Error('Geen foto geladen');timings.normalize=Math.round(performance.now()-t0);
    await vLoadFiles();
    const tv=performance.now();setOcrProgress(20,'Visueel vergelijken…');
    const currentFp=vFingerprint(canvas);scan.legacyFp=currentFp;
    let learned=[];
    try{if(window.CardScoutMemory)learned=await CardScoutMemory.matchLegacy(currentFp,{topK:3,maxDistance:.24});}catch(e){console.warn('learning match',e);}
    if(learned.length){
      const lh=learned[0],lm=learned[1],margin=lm?lm.distance-lh.distance:.2;
      const learnedStrong=(lh.bestDistance<=.075)||((lh.support>=2&&lh.distance<=.105&&margin>=.025));
      if(learnedStrong){scan.visionTop3=learned.map(x=>({key:x.cardKey,name:x.card?.name||'',number:x.card?.number||'',set:x.card?.set||'',distance:Number(x.distance.toFixed(4)),source:'learned'}));scan.visionMeta=visionLastMeta;scan.ocrCalls=0;timings.visual=Math.round(performance.now()-tv);scan.timings=timings;await vApplyCard(lh.card,scan,started,canvas,'learned memory');return;}
    }
    ranked=vRank(currentFp);timings.visual=Math.round(performance.now()-tv);scan.visionTop3=vTopMeta(ranked);scan.visionMeta=visionLastMeta;
    if(vStrong(ranked)){
      const card=vCardForKey(ranked[0].key);if(card){scan.ocrCalls=0;scan.timings=timings;await vApplyCard(card,scan,started,canvas,`visual ${ranked[0].source}`);return;}
    }
    if(!visionIndexReady&&!visionIndexBuilding)vBuildIndex().catch(()=>{});

    // OCR fallback: TITLE FIRST. If the local catalog contains exactly one printing with this name,
    // there is no reason to waste another 10-20 s trying to read a microscopic collector number.
    const worker=await ensureOcrWorker();const guided=true;setOcrProgress(45,'Naam controleren…');const tn=performance.now();
    const nameComposite=core3NameComposite(canvas,guided);
    ...
    const nameRes=await worker.recognize(nameComposite);ocrCalls++;scan.nameRaw=nameRes?.data?.text||'';timings.nameOcr=Math.round(performance.now()-tn);
    const found=findPokemonName(scan.nameRaw);const consensus=!found?core3ConsensusName(scan.nameRaw):'';scan.name=(found?.safe||found?.score>=.84)?found.name:(consensus||'');
    // If visual evidence already points to a candidate with the same recognized name, use it only
    // when the next same-name visual candidate is clearly farther away.
    if(scan.name&&ranked.length){const same=ranked.filter(r=>cardNameMatches(vCardForKey(r.key)||{},scan.name));if(same.length){const margin=same[1]?same[1].distance-same[0].distance:.2;if(same[0].distance<=.26&&margin>=.025){const card=vCardForKey(same[0].key);if(card){scan.ocrCalls=ocrCalls;scan.timings=timings;await vApplyCard(card,scan,started,canvas,'name + visual tie-break');return;}}}}

    // Only ambiguous names need collector-number OCR.
    setOcrProgress(68,'Nummer controleren…');const tb=performance.now();const numberComposite=core3NumberComposite(canvas,guided);
    await worker.setParameters({tessedit_pageseg_mode:'7',tessedit_char_whitelist:'0123456789/',preserve_interword_spaces:'1'});
    const numberRes=await worker.recognize(numberComposite);ocrCalls++;scan.numberRaw=numberRes?.data?.text||'';timings.numberOcr=Math.round(performance.now()-tb);
    const resolved=core3Resolve(scan.nameRaw,scan.numberRaw,langSelect.value);Object.assign(scan,resolved);scan.ocrCalls=ocrCalls;scan.timings=timings;scan.visionTop3=vTopMeta(ranked);scan.visionMeta=visionLastMeta;scan.scanMs=Math.round(performance.now()-started);
    if(scan.card){await vApplyCard(scan.card,scan,started,canvas,'normalized name + collector');return;}

    const finalName=scan.name||'',finalNumber=scan.collector?.number||'',finalSet=scan.set||'';nameInput.value=finalName;numberInput.value=finalNumber;setSelect.value=(finalSet&&[...setSelect.options].some(o=>o.value===finalSet))?finalSet:'AUTO';updateCustomSelects();
    setOcrProgress(100,'Geen zekere exacte match.');if(ocrResult){ocrResult.innerHTML=`<span class="ocrWarn"><strong>Geen zekere match.</strong> CardScout gokt niet.</span><br>${finalName?`Naam: ${escapeHtml(finalName)}<br>`:''}<span class="core3Time">Scantijd: ${(scan.scanMs/1000).toFixed(1)} s</span><br><button type="button" class="debugMarkBad" id="debugMarkBad">⚠️ Foute scan opslaan</button>`;ocrResult.hidden=false;}
    scan.manualLearningUntil=Date.now()+2*60*1000;core3LastScan={scan,canvas};core3LogFailure('onvolledige identificatie',scan,canvas,false);const mark=document.getElementById('debugMarkBad');if(mark)mark.addEventListener('click',()=>{scan.rejected=true;try{window.CardScoutLearningHooks?.markRejected?.();}catch(_){}core3LogFailure('handmatig gemarkeerd als fout',scan,canvas,true);mark.textContent='✓ Opgeslagen in foutlog';mark.disabled=true;});setStatus('Geen zekere match. Vul naam + nummer + set in en open Cardmarket binnen 2 min om deze kaart te leren.','warn');
  }catch(err){console.error(err);scan.reason=String(err?.message||err);scan.scanMs=Math.round(performance.now()-started);scan.ocrCalls=ocrCalls;scan.timings=timings;scan.visionTop3=vTopMeta(ranked);scan.visionMeta=visionLastMeta;if(canvas)core3LogFailure('scanner exception',scan,canvas,false);setOcrProgress(0,'Scan mislukt — opgeslagen in foutlog.');setStatus('Scan mislukt. Foutdetails zijn lokaal opgeslagen.','warn');}
  finally{ocrBusy=false;if(scanPhotoBtn)scanPhotoBtn.disabled=!photoImage;core3RenderDebug();}
}

function core3LearningCardFromFields(){
  const name=(nameInput?.value||'').trim(),number=cleanNumber(numberInput?.value||''),set=(setSelect?.value||'AUTO'),language=langSelect?.value||'EN';
  if(!name||!number||!set||set==='AUTO')return null;
  return knownLookup(language,set,number,name)||{key:`${language.toLowerCase()}|${set.toLowerCase()}|${number.toLowerCase()}|${name.toLowerCase()}`,name,number,set,language,set_name:DATA.sets?.[set]?.label||set,url:null,source:'user'};
}
let core3CorrectionPendingUntil=0;
window.CardScoutLearningHooks={
  beginCorrection(){core3CorrectionPendingUntil=Date.now()+5*60*1000;},
  markRejected(){const s=core3LastScan?.scan;if(s){s.rejected=true;s.manualLearningUntil=0;core3CorrectionPendingUntil=0;if(s.legacyFp&&window.CardScoutMemory)CardScoutMemory.rejectLegacy(s.legacyFp).catch(()=>{});}},
  async acceptOrCorrect(){
    const scan=core3LastScan?.scan;if(!scan?.legacyFp||scan.rejected||!window.CardScoutMemory)return false;
    try{
      const manualWindow=(core3CorrectionPendingUntil>Date.now())||((scan.manualLearningUntil||0)>Date.now());
      if(manualWindow){const c=core3LearningCardFromFields();if(!c)return false;await CardScoutMemory.correctLegacy(c,scan.legacyFp,'manual-correction');core3CorrectionPendingUntil=0;scan.manualLearningUntil=0;return true;}
      if(scan.card){await CardScoutMemory.confirmLegacy(scan.card,scan.legacyFp,'cardmarket-open');return true;}
    }catch(e){console.warn('learning save',e);}
    return false;
  },
  async stats(){return window.CardScoutMemory?CardScoutMemory.stats():null;},
  async backup(){return window.CardScoutMemory?CardScoutMemory.downloadBackup():null;}
};
async function core3RefreshLearningStats(){
  const el=document.getElementById('learningMemoryStats');if(!el||!window.CardScoutMemory)return;
  try{const s=await CardScoutMemory.stats();el.textContent=`Scannergeheugen: ${s.learnedCards} kaarten · ${s.examples} voorbeelden · ${s.corrections} correcties`;}
  catch(_){el.textContent='Scannergeheugen niet beschikbaar';}
}
const learningBackupBtn=document.getElementById('learningBackupBtn');
const learningRestoreBtn=document.getElementById('learningRestoreBtn');
const learningRestoreInput=document.getElementById('learningRestoreInput');
if(learningBackupBtn)learningBackupBtn.addEventListener('click',async()=>{try{await CardScoutMemory?.downloadBackup();await core3RefreshLearningStats();}catch(e){console.warn('learning backup',e);}});
if(learningRestoreBtn&&learningRestoreInput)learningRestoreBtn.addEventListener('click',()=>learningRestoreInput.click());
if(learningRestoreInput)learningRestoreInput.addEventListener('change',async()=>{const f=learningRestoreInput.files?.[0];if(!f)return;try{await CardScoutMemory?.importBackup(await f.text());await core3RefreshLearningStats();setStatus('Scannergeheugen hersteld.','ok');}catch(e){console.warn('learning restore',e);setStatus('Scannergeheugen kon niet worden hersteld.','warn');}finally{learningRestoreInput.value='';}});
setTimeout(()=>core3RefreshLearningStats(),0);
...
if(ocrResult){
  ocrResult.addEventListener('click', e => {
    ...
    updateCustomSelects();
    setStatus(`Set gekozen: ${DATA.sets[set]?.label || set} · ${name} #${number}.`, 'ok');
...
if(debugDownloadBtn) debugDownloadBtn.addEventListener('click',core3DownloadDebug);
if(debugClearBtn) debugClearBtn.addEventListener('click',core3ClearDebug);
core3RenderDebug();

initCustomSelects();
bind();
renderSaved();
setStatus('v142 · RECOVERY · visual + learning memory.', 'ok');
