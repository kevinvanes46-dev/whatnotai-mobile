...
async function loadedArtwork(page,selector,source_id){
 try{
  // Recent can replace its rows while artwork loads. Each poll selects the live
  // container and image together; no element survives across asynchronous calls.
  await page.waitForFunction(({selector,id})=>{
   const target=document.querySelector(selector);if(!target)return false;
   target.scrollIntoView({block:'nearest',behavior:'instant'});
   const img=target.querySelector('img'),expected=window.JPArtwork.resolve({source_id:id,language:'JP'})?.image;
   return !!expected&&!!img&&img.isConnected&&img.getAttribute('src')===expected&&img.complete&&img.naturalWidth>0;
  },{selector,id:source_id.split(' ')[0]},{timeout:5000});
 }catch(error){
  const details=await page.evaluate(selector=>{const el=document.querySelector(selector),img=el?.querySelector('img');return {foundContainer:!!el,foundImg:!!img,src:img?.src||null,complete:img?.complete??null,naturalWidth:img?.naturalWidth??null,targetClass:el?.className,textContent:el?.textContent};},selector);
...
(async()=>{
 ...
 try{
  ...
  for(const ids of Object.values(samples))for(const id of ids)await loadedArtwork(page,'#jp-art-'+id,id);
  ...
  const row=page.locator('.suggestion').filter({hasText:/Swinub/i}).filter({hasText:'Awakening Legends'});await row.waitFor();assert.equal(await row.count(),1);await loadedArtwork(page,'.suggestion .suggestionArt','neo3-038 search');await row.click();
  ...
  await loadedArtwork(page,'#selectedCardArt','neo3-038 selected');const art=await page.locator('#selectedCardArt img').getAttribute('src');assert.equal(art,'https://cdn.artofpkm.com/ki7p3hj3p3phx6tjwmi4av5x0tm6');
  const route=await page.locator('#openBtn').getAttribute('href'),url=new URL(route);assert.equal(url.pathname,'/en/Pokemon/Products/Singles/Awakening-Legends/Swinub-AL');assert.equal(url.searchParams.has('language'),false);assert.equal(url.searchParams.get('minCondition'),'3');
  ...
  await page.reload();await page.locator('#navRecent').click();await loadedArtwork(page,'#recentList .item:first-child','neo3-038 Recent');assert.equal(await page.locator('#recentList .item img').first().getAttribute('src'),art);
  await page.locator('#recentList .useBtn').first().click();await loadedArtwork(page,'#selectedCardArt','neo3-038 restored');assert.equal(await page.locator('#selectedCardArt img').getAttribute('src'),art);
  ...
  await page.locator('#navCollection').click();await loadedArtwork(page,'#collectionList','neo3-038 collection');assert.equal(await page.locator('#collectionList img').first().getAttribute('src'),art);
