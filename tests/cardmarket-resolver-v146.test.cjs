...
test('API timeout <= 3000ms even if fetch ignores abort',async()=>{let delay,cb;const {c}=context({fetch:()=>new Promise(()=>{}),setTimeout:(fn,ms)=>{delay=ms;cb=fn;return 1;},clearTimeout:()=>{}});const pending=c.resolveCardmarketRoute(azumarill,fallback(c,azumarill));assert.equal(delay,3000);cb();isFallback(await pending,'Azumarill');});
...
test('Expired, future, malformed and non-array cache entries are ignored',async()=>{for(const raw of [JSON.stringify({oops:true}),...[-31*86400000,60000].map(age=>JSON.stringify([{source_id:'ex11-19',url:AZU,timestamp:Date.now()+age}])),JSON.stringify([{source_id:'ex11-19',url:'http://evil.test/',timestamp:Date.now()}])]){const {c,calls}=context({storage:[[CACHE,raw]]});isFallback(await c.resolveCardmarketRoute(azumarill,fallback(c,azumarill)),'Azumarill');assert.equal(calls.length,1);}});
