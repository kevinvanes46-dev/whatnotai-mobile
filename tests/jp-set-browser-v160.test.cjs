...
(async()=>{
 ...
 try{
  ...
  for(const [query,id] of checks){
   ...
   if(source==='PMCG6')assert.equal(new URL(await page.locator('#openBtn').getAttribute('href')).pathname,'/en/Pokemon/Products/Singles/Challenge-from-the-Darkness/Kogas-Weedle-CFTD');
