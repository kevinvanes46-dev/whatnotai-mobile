...
(() => {
  ...
  const SEARCH_BUILD = '133-search-collection';
  ...
  const STAMP_TERMS = new Set(['stamp','stamped','stempel','gestempeld','setstamp']);
  ...
  function appendSuggestionBatch(){
    ...
    for(let i=renderedSuggestionCount;i<end;i++){
      ...
      btn.addEventListener('click', () => {
        ...
        showToast(stampedSelected ? `${card.name}: stamped-era kaart geselecteerd` : (isRemote && lang === 'JP'
  ...
  function renderSuggestions(query){
    ...
    const ignored = new Set(['en','eng','english','jp','jpn','japanese','nm','ex','gd','pl','1st','first','edition','normal','normaal','stamp','stamped','stempel','gestempeld','setstamp']);
