...
// ---------------- Photo + Local OCR + Guided Scan v71 ----------------
...
const SET_TOTAL_HINTS = {
  ...
  144:['SKYRIDGE'],147:['AQUAPOLIS'],165:['EXPEDITION'],92:['EX LEGEND MAKER'],12:['EX TRAINER KIT 2']
};
...
function extractCollectorNumberLoose(text, candidateCards=[]){
  // v72: als de naam al bekend is, vergelijk de OCR eerst met de bekende kaartnummers
  // voor die naam. Dit voorkomt bv. Horsea 62/111 -> OCR 2/111.
  // Een duidelijke kandidaatmatch mag een weggevallen eerste cijfer herstellen.
  if(candidateCards && candidateCards.length){
    const candidateFirst=matchCollectorToCandidates(text,candidateCards);
    if(candidateFirst && candidateFirst.score>=0.76){
      return {number:candidateFirst.number,total:candidateFirst.total,source:`candidate ${candidateFirst.reason}`,candidateCard:candidateFirst.card,score:candidateFirst.score};
    }
  }
...
async function scanCollectorEvidence(worker, canvas, guided, candidateCards=[]){
  // v72 NAME + SET RECONCILE: begin breed, zoom daarna steeds verder in op de echte rechteronderhoek.
  // We bewaren alle OCR-snippers en matchen die ook tegen verwachte nummer/set-combinaties van de herkende naam.
  const rects = guided ? [
    {left:.62, top:.81, width:.37, height:.18},
    {left:.50, top:.77, width:.49, height:.22},
    {left:.36, top:.74, width:.63, height:.25},
    {left:.18, top:.82, width:.81, height:.17}
  ] : [
    {left:.60, top:.79, width:.39, height:.20},
    {left:.46, top:.73, width:.53, height:.26},
...
async function scanCurrentPhoto(){
  ...
  try{
    ...
    let localInference=collectorEvidence.candidateCard || null;
    ...
    if(displayName && canAutofill){
      ...
      // v72 belangrijkste veiligheidsregel:
      // naam + eenduidig gedetecteerde set + exact één lokale kaart wint van een
      // afgesneden OCR-numerator. Voorbeeld: OCR leest 2/111, maar Horsea heeft in
      // Neo Genesis lokaal alleen #62. Dan gebruiken we #62 en NIET NG2.
      if(inferredSet && setCandidates.length===1){
      ...
      }else if(!localInference && !collector && allNameCandidates.length===1){
    ...
    if(ocrResult){
      ...
      if(inferredSet) pieces.push(`<strong>Set-hint:</strong> ${escapeHtml(EMBEDDED_DATA.sets[inferredSet]?.label || inferredSet)}${collectorEvidence?.evidence && collectorEvidence.evidence!=='none' ? ` <small>(${escapeHtml(collectorEvidence.evidence)})</small>` : (!collector && denominatorHint ? ' <small>(set-totaal gelezen)</small>' : '')}`);
