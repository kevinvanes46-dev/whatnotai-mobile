  ...
  visionLoadPromise=(async()=>{
    try{
      const [a,b]=await Promise.all([
        fetch('visual-seeds.json?build=141',{cache:'no-cache'}).then(r=>r.json()),
        fetch('visual-catalog.json?build=141',{cache:'no-cache'}).then(r=>r.json())
      ]);
...
bind();
renderSaved();
setStatus('v141 · CLICK FIX · visual + learning memory.', 'ok');
