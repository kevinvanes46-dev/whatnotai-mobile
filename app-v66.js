...
function findPokemonName(text){
  ...
  const direct = candidates.filter(n => low.includes(n.toLowerCase())).sort((a,b)=>b.length-a.length)[0];
  if(direct) return {name:direct, confidence:'hoog'};
  const words = clean.split(/\s+/).filter(w=>w.length>=3);
  let best=null;
  for(const name of candidates){
    const nn=simpleNorm(name); if(nn.length<3) continue;
    for(const word of words){
      const ww=simpleNorm(word); if(Math.abs(ww.length-nn.length)>2) continue;
  ...
  return best ? {name:best.name, confidence:best.score>=0.86?'redelijk':'laag'} : null;
}
function extractCollectorNumber(text){
  ...
  const frac=[...t.matchAll(/\b(\d{1,3})\s*[\/|]\s*(\d{1,3})\b/g)]
    ...
    .filter(x=>x.total>=20 && x.total<=250 && Number(x.number)<=x.total+20);
  return frac[0] || null;
}
...
async function scanCurrentPhoto(){
  ...
  try{
    ...
    const topRect={left:0,top:0,width:canvas.width,height:Math.max(1,Math.round(canvas.height*0.34))};
    setOcrProgress(15,'Naam lezen…');
    await worker.setParameters({tessedit_pageseg_mode:'6'});
    const top=await worker.recognize(canvas,{rectangle:topRect});
    let topText=top?.data?.text || '';
    let found=findPokemonName(topText);
    let fullText=topText;
    if(!found){
      setOcrProgress(48,'Naam niet zeker — tweede scan…');
      const full=await worker.recognize(canvas);
      fullText=full?.data?.text || '';
      found=findPokemonName(fullText);
    }
    setOcrProgress(72,'Kaartnummer proberen…');
    ...
    fullText += ' ' + bottomText;
    const collector=extractCollectorNumber(fullText);
    const inferredSet=collector ? inferSetFromTotal(collector.total) : null;
    ...
    const local=found ? findKnownLocal(found.name, collector?.number, inferredSet) : null;
    if(ocrResult){
      const pieces=[];
      if(found) pieces.push(`<strong>Naam:</strong> ${escapeHtml(found.name)} <small>(${found.confidence})</small>`);
      else pieces.push('<span class="ocrWarn">Naam nog niet betrouwbaar herkend.</span>');
      ...
      pieces.push('<span class="ocrRaw">OCR: '+escapeHtml(normalizeScanText(fullText).slice(0,220))+'</span>');
      ocrResult.innerHTML=pieces.join('<br>');
      ocrResult.hidden=false;
    }
    if(found){
      setStatus(`Scan: ${found.name}${collector?` #${collector.number}`:''}. Controleer naam, nummer, set en staat.`, 'ok');
      const panel=document.querySelector('.formPanel');
      if(panel) setTimeout(()=>panel.scrollIntoView({behavior:'smooth',block:'start'}),500);
    }else{
...
if(takePhotoBtn && cameraInput){
  ...
  cameraInput.addEventListener('change', () => loadPhotoFile(cameraInput.files && cameraInput.files[0]));
}
if(choosePhotoBtn && galleryInput){
  choosePhotoBtn.addEventListener('click', () => galleryInput.click());
  galleryInput.addEventListener('change', () => loadPhotoFile(galleryInput.files && galleryInput.files[0]));
}
if(rotateLeftBtn){
...
bind();
renderSaved();
setStatus('v66 · OCR Lab · DB 243.', 'ok');
