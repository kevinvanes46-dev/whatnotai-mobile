...
// ============================================================
const CORE3_BUILD = 100;
const CORE3_DEBUG_KEY = 'cardscout_core3_failed_scans_v100';
...
async function scanCurrentPhoto(){
  ...
  let canvas=null, scan=null;
  try{
    setOcrProgress(4,'Naam, nummer en set voorbereiden…');
    canvas=makeOcrCanvas();
    if(!canvas) throw new Error('Geen foto geladen');
    const worker=await ensureOcrWorker();
    const guided=photoGuidedCrop || photoAutoCropped;

    // OCR CALL #1 — ONLY THE CARD NAME/TITLE STRIP.
    setOcrProgress(25,'Kaartnaam lezen…');
    ...
    const nameRes=await worker.recognize(nameComposite);
    const nameText=nameRes?.data?.text||'';

    // OCR CALL #2 — ONLY THE LOWER COLLECTOR-NUMBER STRIP.
    setOcrProgress(60,'Kaartnummer lezen…');
    const numberComposite=core3NumberComposite(canvas,guided);
    await worker.setParameters({
      tessedit_pageseg_mode:'6',
      tessedit_char_whitelist:'0123456789/',
      preserve_interword_spaces:'1'
    });
    const numberRes=await worker.recognize(numberComposite);
    const numberText=numberRes?.data?.text||'';

    scan=core3Resolve(nameText,numberText,langSelect.value);
    scan.scanMs=Math.round(performance.now()-started);

    // Set symbol is a tie-breaker only. It may fill a missing set, never overwrite exact DB evidence.
    if(!scan.card && scan.name){
      const candidates=localCandidatesByName(scan.name,langSelect.value);
      const symbol=detectSetSymbolLocal(canvas,candidates);
      if(symbol?.set){
        const scoped=candidates.filter(c=>c.set===symbol.set);
        if(scoped.length===1){
          scan.card=scoped[0];scan.set=scoped[0].set;
          scan.collector={number:cleanNumber(scoped[0].number),total:setTotalForSet(scoped[0].set),source:'name+set-symbol',candidateCard:scoped[0]};
          scan.reason=(scan.reason?scan.reason+', ':'')+'name+set-symbol';
        }else if(!scan.set){scan.set=symbol.set;scan.reason=(scan.reason?scan.reason+', ':'')+'set-symbol';}
      }
    }

    // Auto-fill ONLY fields proven by this scan.
    const finalName=scan.card?.name||scan.name||'';
    const finalNumber=scan.card?.number||scan.collector?.number||'';
    const finalSet=scan.card?.set||scan.set||'';
    nameInput.value=finalName;
    numberInput.value=finalNumber;
    setSelect.value=(finalSet && [...setSelect.options].some(o=>o.value===finalSet))?finalSet:'AUTO';
    updateCustomSelects();

    const complete=!!(finalName && finalNumber && finalSet);
    setOcrProgress(100,complete?'Scan klaar.':'Scan onzeker — opgeslagen in foutlog.');
    if(ocrResult){
      const setLabel=finalSet?(EMBEDDED_DATA.sets[finalSet]?.label||finalSet):'';
      const total=scan.collector?.total||setTotalForSet(finalSet)||'';
      ocrResult.innerHTML=[
        finalName?`<strong>Naam:</strong> ${escapeHtml(finalName)}`:'<span class="ocrWarn"><strong>Naam:</strong> niet zeker</span>',
        finalNumber?`<strong>Kaartnummer:</strong> ${escapeHtml(finalNumber)}${total?'/'+escapeHtml(total):''}`:'<span class="ocrWarn"><strong>Kaartnummer:</strong> niet zeker</span>',
        finalSet?`<strong>Set:</strong> ${escapeHtml(setLabel)}`:'<span class="ocrWarn"><strong>Set:</strong> niet zeker</span>',
        `<span class="core3Time">Scantijd: ${(scan.scanMs/1000).toFixed(1)} s</span>`,
        `<button type="button" class="debugMarkBad" id="debugMarkBad">⚠️ Foute scan opslaan</button>`
      ].join('<br>');
      ocrResult.hidden=false;
    }
    core3LastScan={scan,canvas};
    const mark=document.getElementById('debugMarkBad');
    if(mark) mark.addEventListener('click',()=>{core3LogFailure('handmatig gemarkeerd als fout',scan,canvas,true);mark.textContent='✓ Opgeslagen in foutlog';mark.disabled=true;});

    if(!complete){
      core3LogFailure('onvolledige identificatie',scan,canvas,false);
      setStatus('Scan niet zeker. Alleen naam + kaartnummer + set worden gebruikt; geen kaarttekst wordt gelezen.', 'warn');
    }else{
      setStatus(`Scan: ${finalName} #${finalNumber} · ${EMBEDDED_DATA.sets[finalSet]?.label||finalSet}.`, 'ok');
      // Build the existing CM route immediately; do not open it automatically.
      try{ await makeLink(false); }catch(_){ /* scan result remains usable even if link construction fails */ }
    }
    if(scanEditionQuick) scanEditionQuick.hidden=false;
  }catch(err){
    console.error(err);
    const failed={name:'',collector:null,total:null,set:'',card:null,reason:String(err?.message||err),nameRaw:'',numberRaw:'',scanMs:Math.round(performance.now()-started)};
    if(canvas) core3LogFailure('scanner exception',failed,canvas,false);
    setOcrProgress(0,'Scan mislukt — opgeslagen in foutlog.');
    if(ocrResult){ocrResult.innerHTML='<span class="ocrWarn">Scanner kon naam, kaartnummer en set niet betrouwbaar bepalen.</span>';ocrResult.hidden=false;}
    setStatus('Scan mislukt. Foutdetails zijn lokaal opgeslagen.', 'warn');
  }finally{
    ocrBusy=false;if(scanPhotoBtn)scanPhotoBtn.disabled=!photoImage;core3RenderDebug();
  }
}
...
bind();
renderSaved();
setStatus('v100 · CORE3 STABLE · alleen naam + kaartnummer + set.', 'ok');
