...
// ---------------- v81 FOCUSED SCAN: preferences + Smart Search + cached DP/POP catalog ----------------
...
// ---------------- Photo + Local OCR + Guided Scan · v81 FOCUSED SCAN ----------------
...
function makeOcrCanvas(){
  ...
  // v81 FOCUSED SCAN: het live kader is een richtlijn, niet de echte kaartgrens.
...
// --- v81 FOCUSED SCAN -------------------------------------------------------
// Doel: geen full-card OCR meer. Per kaart slechts:
//   1) naamstrook linksboven,
//   2) collector number rechtsonder,
//   3) lokaal set-symbool als tie-breaker.
// Hierdoor verdwijnen de 30-40 s brede OCR-fallbacks en hallucinaties zoals 3/23.
function makeFocusedScanCrop(source, fracRect, options={}){
  if(!source) return null;
  const rect={
    left:Math.round(source.width*fracRect.left),
    top:Math.round(source.height*fracRect.top),
    width:Math.round(source.width*fracRect.width),
    height:Math.round(source.height*fracRect.height)
  };
  ...
  return makeEnhancedCrop(source,{left:sx,top:sy,width:sw,height:sh},{
    zoom,
...
function stackScanCrops(crops,gap=18){
  const valid=(crops||[]).filter(Boolean);
  if(!valid.length) return null;
  const w=Math.max(...valid.map(c=>c.width));
  const h=valid.reduce((s,c)=>s+c.height,0)+gap*(valid.length-1);
  const out=document.createElement('canvas'); out.width=w; out.height=h;
  const ctx=out.getContext('2d',{willReadFrequently:true});
  ctx.fillStyle='#fff'; ctx.fillRect(0,0,w,h);
  let y=0;
  for(const c of valid){ ctx.drawImage(c,0,y,c.width,c.height); y+=c.height+gap; }
  return out;
}
function focusedNameComposite(canvas){
  // Top strip is deliberately wide enough for Dark X / Lv. / Delta Species names.
  const rect={left:.025,top:.015,width:.74,height:.135};
  return stackScanCrops([
    makeFocusedScanCrop(canvas,rect,{targetWidth:1350,contrast:1.55,maxZoom:4.5}),
    makeFocusedScanCrop(canvas,rect,{targetWidth:1350,contrast:2.0,threshold:165,maxZoom:4.5})
  ]);
}
function focusedNumberComposite(canvas){
  // Only the printed collector-number corner. Larger than before so 62/111 is not clipped.
  const rect={left:.59,top:.865,width:.405,height:.13};
  return stackScanCrops([
    makeFocusedScanCrop(canvas,rect,{targetWidth:1550,contrast:1.85,maxZoom:7}),
    makeFocusedScanCrop(canvas,rect,{targetWidth:1550,contrast:2.15,threshold:155,maxZoom:7}),
    makeFocusedScanCrop(canvas,rect,{targetWidth:1550,contrast:2.15,threshold:185,maxZoom:7})
  ],14);
}
function focusedCollectorFromText(text,candidateCards=[]){
  ...
  const add=(x,source)=>{
    if(!x) return;
    const key=`${cleanNumber(x.number)}|${Number(x.total)}`;
    const card=x.candidateCard || null;
    const score=Number(x.score||0);
    seen.push({...x,key,card,score,source:source||x.source||'ocr'});
  };
  // First evaluate each visual variant separately; this avoids one noisy line poisoning another.
  for(const line of lines){
    add(extractCollectorNumberLoose(line,candidateCards),'focused line');
    const partial=matchPartialCollectorToCandidates(line,candidateCards);
    if(partial) add({number:partial.number,total:partial.total,candidateCard:partial.card,score:partial.score},partial.reason);
  }
  // Then one combined candidate match for split digits such as "6 2 / 1 1 1".
  const combined=extractCollectorNumberLoose(raw,candidateCards);
  add(combined,'focused combined');
  const matched=matchCollectorToCandidates(raw,candidateCards);
  if(matched) add({number:matched.number,total:matched.total,candidateCard:matched.card,score:matched.score},matched.reason);

  if(!seen.length) return null;
  const grouped=new Map();
  for(const x of seen){
    const g=grouped.get(x.key)||{...x,count:0,maxScore:0,card:x.card||null};
    g.count++; g.maxScore=Math.max(g.maxScore,x.score||0);
    if(!g.card&&x.card) g.card=x.card;
    grouped.set(x.key,g);
  }
  const ranked=[...grouped.values()].sort((a,b)=>{
    const ac=(a.card?2:0)+(a.count>=2?1:0)+(a.maxScore>=.97?1:0);
    const bc=(b.card?2:0)+(b.count>=2?1:0)+(b.maxScore>=.97?1:0);
    return bc-ac || b.count-a.count || b.maxScore-a.maxScore;
  });
  const best=ranked[0];
  // A name-scoped DB card or repeated reading is safe. A single unsupported hallucination is not.
  const supported=Object.prototype.hasOwnProperty.call(SET_TOTAL_HINTS,Number(best.total));
  if(best.card || best.count>=2 || (supported && best.maxScore>=.97)){
    return {number:String(parseInt(best.number,10)),total:Number(best.total),source:best.source,candidateCard:best.card||null,score:best.maxScore,count:best.count};
  }
  return null;
}
function uniqueDataCollector(number,total,lang='EN'){
  const n=cleanNumber(number), t=Number(total);
  const source=(DATA&&DATA.knownCards)||EMBEDDED_DATA.knownCards||[];
  const hits=source.filter(c=>c.language===lang && cleanNumber(c.number)===n && setTotalForSet(c.set)===t);
  return hits.length===1 ? hits[0] : null;
}
// --- end v81 focused helpers ------------------------------------------------
async function scanCurrentPhoto(){
  ...
  if(ocrResult){ ocrResult.hidden=true; ocrResult.textContent=''; }
  try{
    setOcrProgress(4,'Kaart uitlijnen…');
    const canvas=makeOcrCanvas();
    if(!canvas) throw new Error('Geen foto geladen');
    const worker=await ensureOcrWorker();

    // 1/3 — NAME. Eén OCR-call met twee beeldvarianten, nooit de hele kaart.
    setOcrProgress(18,'Naam lezen…');
    const nameCanvas=focusedNameComposite(canvas);
    await worker.setParameters({
      tessedit_pageseg_mode:'6',
      tessedit_char_whitelist:"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'-. !&"
    });
    const nameScan=await worker.recognize(nameCanvas||canvas);
    const nameText=nameScan?.data?.text || '';
    const trainerGuess=findTrainerCandidate(nameText);
    const trainerDetected=looksLikeTrainer(nameText) || !!trainerGuess;
    const trainerTitle=trainerGuess || (trainerDetected ? extractTrainerTitle(nameText) : '');
    let found=trainerDetected ? null : findPokemonName(nameText);
    let provisionalName=trainerTitle || found?.name || '';
    let scanNameCandidates=provisionalName ? localCandidatesByName(provisionalName,langSelect.value) : [];

    // 2/3 — NUMBER. Eén OCR-call op drie vergrote varianten van uitsluitend de rechteronderhoek.
    setOcrProgress(48,'Kaartnummer lezen…');
    const numberCanvas=focusedNumberComposite(canvas);
    await worker.setParameters({
      tessedit_pageseg_mode:'6',
      tessedit_char_whitelist:'0123456789/',
      preserve_interword_spaces:'1'
    });
    const numberScan=await worker.recognize(numberCanvas||canvas);
    const numberText=numberScan?.data?.text || '';
    let collector=focusedCollectorFromText(numberText,scanNameCandidates);
    let collectorCard=collector?.candidateCard || null;

    // If the exact collector is globally unique in the loaded local catalog it may rescue a weak name.
    const globalCollectorCard=collector ? uniqueDataCollector(collector.number,collector.total,langSelect.value) : null;
    if(!collectorCard && globalCollectorCard) collectorCard=globalCollectorCard;

    // Determine set from strong number evidence first. This can never be overwritten by a weaker symbol guess.
    let inferredSet=null;
    if(collectorCard) inferredSet=collectorCard.set;
    if(!inferredSet && collector) inferredSet=inferSetFromTotal(collector.total,scanNameCandidates);
    const specialHint=specialScanHint(provisionalName,collector);
    if(!inferredSet && specialHint?.set) inferredSet=specialHint.set;

    // 3/3 — SET SYMBOL. Pure local pixel matching, only when number/name did not already settle the set.
    setOcrProgress(74,'Set-symbool controleren…');
    let symbolEvidence=null;
    if(!trainerDetected && !inferredSet){
      symbolEvidence=detectSetSymbolLocal(canvas,scanNameCandidates);
      if(symbolEvidence?.set) inferredSet=symbolEvidence.set;
    }

    // Reconcile the three independent signals against the catalog. No guessing.
    let displayName=trainerTitle || found?.name || '';
    let canAutofill=!!trainerTitle || !!found?.safe;
    let localInference=collectorCard || globalCollectorCard || null;
    ...
    if(displayName){
      ...
      const exactCandidates=collector ? setCandidates.filter(c=>
        cleanNumber(c.number)===cleanNumber(collector.number) && setTotalForSet(c.set)===Number(collector.total)
      ) : [];

      if(exactCandidates.length===1){
        localInference=exactCandidates[0]; inferredSet=localInference.set; displayName=localInference.name; canAutofill=true;
      }else if(inferredSet && setCandidates.length===1 && (canAutofill || (found&&found.score>=.72))){
        // Name + confirmed set uniquely determines one local card. This repairs clipped OCR like 62/1.
        localInference=setCandidates[0]; displayName=localInference.name; canAutofill=true;
        if(collector && cleanNumber(localInference.number)!==cleanNumber(collector.number)){
          collectorRejected=true; rejectedCollectorNumber=collector.number;
        }
      }else if(!collector && allNameCandidates.length===1 && canAutofill){
        localInference=allNameCandidates[0]; inferredSet=localInference.set;
      }
    }

    if(!canAutofill && localInference){
      // Number can rescue the name only when the collector pair is unique in our catalog.
      displayName=localInference.name; inferredSet=localInference.set; canAutofill=true;
    }

    // If a strong local card conflicts with the OCR number, trust the catalog reconciliation.
    if(localInference && collector && inferredSet===localInference.set && cleanNumber(localInference.number)!==cleanNumber(collector.number)){
      collectorRejected=true; rejectedCollectorNumber=collector.number;
    }
    ...
    // This scan always replaces previous scan fields; no stale values.
    nameInput.value=(displayName&&canAutofill)?displayName:'';
    numberInput.value=resolvedNumber;
    ...
    setOcrProgress(100,'Scan klaar. Naam, nummer en set gecontroleerd.');
    if(ocrResult){
      const pieces=[];
      if(trainerDetected){
        pieces.push('<strong>Type:</strong> Trainerkaart');
        pieces.push(trainerTitle?`<strong>Naam:</strong> ${escapeHtml(trainerTitle)}`:'<span class="ocrWarn">Trainernaam niet zeker.</span>');
      }else if(found){
        pieces.push(`<strong>Naam:</strong> ${escapeHtml(displayName||found.name)} <small>(${found.safe?'hoog':'mogelijk'})</small>`);
      }else if(localInference){
        pieces.push(`<strong>Naam:</strong> ${escapeHtml(localInference.name)} <small>(hersteld uit nummer/set)</small>`);
      }else pieces.push('<span class="ocrWarn">Naam nog niet zeker.</span>');

      if(resolvedNumber){
        const total=localInference?setTotalForSet(localInference.set):(collector?.total||null);
        pieces.push(`<strong>Nummer:</strong> ${escapeHtml(resolvedNumber)}${total?'/'+escapeHtml(total):''}${collectorRejected?' <small>(OCR gecorrigeerd)</small>':''}`);
      }else pieces.push('<span class="ocrWarn">Kaartnummer nog niet zeker.</span>');

      if(inferredSet){
        let why='';
        if(collectorCard||globalCollectorCard) why=' <small>(nummer + database)</small>';
        else if(collector) why=' <small>(set-totaal)</small>';
        else if(symbolEvidence?.set===inferredSet) why=` <small>(set-symbool · ${Math.round(symbolEvidence.score*100)}%)</small>`;
        pieces.push(`<strong>Set:</strong> ${escapeHtml(EMBEDDED_DATA.sets[inferredSet]?.label||inferredSet)}${why}`);
      }else pieces.push('<span class="ocrWarn">Set nog niet zeker.</span>');

      pieces.push(`<span class="ocrEditionOff">Editie: handmatig · ${editionSelect?.value==='1ST'?'<b>1ST EDITION</b>':'<b>Normaal</b>'}.</span>`);
      if(localInference) pieces.push('<strong>Lokale match:</strong> gevonden in CardScout DB');
      if(photoAutoCropped) pieces.push('<strong>Foto:</strong> kaartgrens automatisch uitgelijnd');
      else if(photoGuidedCrop) pieces.push('<strong>Foto:</strong> camerakader gebruikt');
      else pieces.push('<strong>Foto:</strong> volledige foto gebruikt');
      pieces.push(`<strong>Scantijd:</strong> ${seconds} s`);
      pieces.push(`<span class="ocrRaw">Naam OCR: ${escapeHtml(normalizeScanText(nameText).slice(0,100))}<br>Nummer OCR: ${escapeHtml(normalizeScanText(numberText).slice(0,100))}</span>`);
      ocrResult.innerHTML=pieces.join('<br>');
      ocrResult.hidden=false;
      if(scanEditionQuick) scanEditionQuick.hidden=false;
    }

    if(displayName&&canAutofill&&resolvedNumber&&inferredSet){
      // Auto-fill and build the Cardmarket route; user still taps Open CM deliberately.
      ...
      setStatus(`Scan klaar in ${seconds} s: ${displayName} #${resolvedNumber} · ${EMBEDDED_DATA.sets[inferredSet]?.label||inferredSet}.`, 'ok');
    }else{
      setStatus(`Scan klaar in ${seconds} s. Alleen zekere gegevens zijn ingevuld.`, 'warn');
    }
  }catch(err){
    console.error(err);
    setOcrProgress(0,'Scan mislukt.');
    if(ocrResult){ocrResult.innerHTML='<span class="ocrWarn">Scanner kon niet starten. Probeer opnieuw.</span>';ocrResult.hidden=false;}
    setStatus('Scanner kon niet starten. Handmatig zoeken blijft beschikbaar.','warn');
  }finally{
    ocrBusy=false;
...
// v81: laad OCR alvast rustig op de achtergrond zodat de eerste echte scan niet voor model-init hoeft te wachten.
