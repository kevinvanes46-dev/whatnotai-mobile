...
function core3LogFailure(reason,scan,canvas,manual=false){
  ...
  const entry={
    ...
    scanner:'VISION CORE v140 FOUNDATION',ocrCalls:scan?.ocrCalls??null,timings:scan?.timings||null,
  ...
  visionLoadPromise=(async()=>{
    try{
      const [a,b]=await Promise.all([
        fetch('visual-seeds.json?build=140',{cache:'no-cache'}).then(r=>r.json()),
        fetch('visual-catalog.json?build=140',{cache:'no-cache'}).then(r=>r.json())
      ]);
    ...
    const needed=Math.max(1,visionCatalog.length);
    visionIndexReady=vOfficialCoverage()>=needed;
  })();
...
async function vBuildIndex(){
  await vLoadFiles();
  if(visionIndexBuilding||visionIndexReady||!visionCatalog.length)return;
  ...
  const needed=Math.max(1,visionCatalog.length);
  visionIndexReady=vOfficialCoverage()>=needed;visionIndexBuilding=false;
}
...
function vStrong(rank){
  ...
  const a=rank[0],b=rank[1],margin=b?b.distance-a.distance:.2;
  // Automatic acceptance only from the complete official catalog. Seed-only cards must be verified/learned.
  if(a.source!=='official')return false;
  return a.distance<=.13||(a.distance<=.20&&margin>=.035)||(a.distance<=.22&&margin>=.05);
}
...
bind();
renderSaved();
setStatus('v140 · FOUNDATION · visual + learning memory.', 'ok');
