...
function pokemonCandidates(){
  ...
  const known = (EMBEDDED_DATA.knownCards || []).flatMap(c=>[c.name,...(c.aliases||[])]);
...
// ============================================================
const CORE3_BUILD = 130;
const CORE3_DEBUG_KEY = 'cardscout_vision_failed_scans_v130';
...
function core3ExactCard(name,number,total,lang='EN'){
  ...
  const hits=(EMBEDDED_DATA.knownCards||[]).filter(c=>
...
function core3LogFailure(reason,scan,canvas,manual=false){
  ...
  const entry={
    ...
    scanner:'VISION CORE v130',ocrCalls:scan?.ocrCalls??null,timings:scan?.timings||null,
...
const VISION_CACHE_KEY='cardscout.visual.v130.refs';
...
function vCardForKey(key){return (EMBEDDED_DATA.knownCards||[]).find(c=>c.key===key)||(DATA?.knownCards||[]).find(c=>c.key===key)||null;}
...
async function vLoadFiles(){if(visionLoadPromise)return visionLoadPromise;visionLoadPromise=(async()=>{try{const [a,b]=await Promise.all([fetch('visual-seeds.json?build=130',{cache:'no-cache'}).then(r=>r.json()),fetch('visual-catalog.json?build=130',{cache:'no-cache'}).then(r=>r.json())]);visionSeeds=a.seeds||[];visionCatalog=b.refs||[];}catch(e){console.warn('vision files',e);}try{const c=JSON.parse(localStorage.getItem(VISION_CACHE_KEY)||'null');if(c?.version==='v130'&&Array.isArray(c.refs))visionRefs=c.refs;}catch(_){}visionIndexReady=visionRefs.length>=Math.max(1,Math.floor(visionCatalog.length*.85));})();return visionLoadPromise;}
...
function vSaveCache(){try{localStorage.setItem(VISION_CACHE_KEY,JSON.stringify({version:'v130',refs:visionRefs}));}catch(e){console.warn('vision cache',e);}}
async function vBuildIndex(){await vLoadFiles();if(visionIndexBuilding||visionIndexReady||!visionCatalog.length)return;visionIndexBuilding=true;const have=new Set(visionRefs.map(x=>x.key)),priority=new Set(visionSeeds.map(x=>x.key)),todo=visionCatalog.filter(x=>!have.has(x.key)).sort((a,b)=>(priority.has(b.key)?1:0)-(priority.has(a.key)?1:0));let dirty=0,done=0;const workers=Array.from({length:4},async()=>{while(todo.length){const r=todo.shift();if(!r)break;try{const im=await vImage(r.url);visionRefs.push({key:r.key,fp:vFingerprint(im)});dirty++;}catch(_){}done++;if(dirty>=16){vSaveCache();dirty=0;}if(done%12===0)await new Promise(z=>setTimeout(z,0));}});await Promise.all(workers);vSaveCache();visionIndexReady=visionRefs.length>=Math.max(1,Math.floor(visionCatalog.length*.82));visionIndexBuilding=false;}
function vRank(fp){const best=new Map();for(const r of [...visionSeeds,...visionRefs]){if(!r?.key||!r?.fp)continue;const d=vDistance(fp,r.fp),old=best.get(r.key);if(!old||d<old.distance)best.set(r.key,{key:r.key,distance:d,source:visionSeeds.includes(r)?'seed':'official'});}return [...best.values()].sort((a,b)=>a.distance-b.distance);}
function vStrong(rank){if(!rank.length)return false;const a=rank[0],b=rank[1],margin=b?b.distance-a.distance:.2;return a.distance<=.13||(a.distance<=.20&&margin>=.035)||(a.source==='official'&&a.distance<=.22&&margin>=.04)||(a.source==='seed'&&a.distance<=.245&&margin>=.085);}
...
async function vApplyCard(card,scan,started,canvas,reason){if(!card)return false;scan=scan||{};scan.card=card;scan.name=card.name;scan.set=card.set;scan.collector={number:cleanNumber(card.number),total:setTotalForSet(card.set),source:reason,candidateCard:card};scan.reason=reason;scan.scanMs=Math.round(performance.now()-started);nameInput.value=card.name;numberInput.value=cleanNumber(card.number);setSelect.value=[...setSelect.options].some(o=>o.value===card.set)?card.set:'AUTO';updateCustomSelects();setOcrProgress(100,'Kaart gevonden.');if(ocrResult){ocrResult.innerHTML=`<strong>Naam:</strong> ${escapeHtml(card.name)}<br><strong>Kaartnummer:</strong> ${escapeHtml(cleanNumber(card.number))}/${escapeHtml(String(setTotalForSet(card.set)||''))}<br><strong>Set:</strong> ${escapeHtml(EMBEDDED_DATA.sets[card.set]?.label||card.set)}<br><span class="core3Time">Scantijd: ${(scan.scanMs/1000).toFixed(1)} s · ${escapeHtml(reason)}</span><br><button type="button" class="debugMarkBad" id="debugMarkBad">⚠️ Foute scan opslaan</button>`;ocrResult.hidden=false;}core3LastScan={scan,canvas};const mark=document.getElementById('debugMarkBad');if(mark)mark.addEventListener('click',()=>{core3LogFailure('handmatig gemarkeerd als fout',scan,canvas,true);mark.textContent='✓ Opgeslagen in foutlog';mark.disabled=true;});setStatus(`Scan: ${card.name} #${cleanNumber(card.number)} · ${EMBEDDED_DATA.sets[card.set]?.label||card.set}.`,'ok');try{await makeLink(false);}catch(_){}if(scanEditionQuick)scanEditionQuick.hidden=false;return true;}
...
if('requestIdleCallback'in window)requestIdleCallback(()=>{vLoadFiles().catch(()=>{});},{timeout:1200});else setTimeout(()=>{vLoadFiles().catch(()=>{});},900);
...
async function scanCurrentPhoto(){
  ...
  try{
    ...
    await vLoadFiles();const tv=performance.now();setOcrProgress(20,'Visueel vergelijken…');ranked=vRank(vFingerprint(canvas));timings.visual=Math.round(performance.now()-tv);scan.visionTop3=vTopMeta(ranked);scan.visionMeta=visionLastMeta;
    if(vStrong(ranked)){
    ...
    core3LastScan={scan,canvas};core3LogFailure('onvolledige identificatie',scan,canvas,false);const mark=document.getElementById('debugMarkBad');if(mark)mark.addEventListener('click',()=>{core3LogFailure('handmatig gemarkeerd als fout',scan,canvas,true);mark.textContent='✓ Opgeslagen in foutlog';mark.disabled=true;});setStatus('Geen zekere match. Opgeslagen in foutlog.','warn');
...
bind();
renderSaved();
setStatus('v130 · VISION CORE · visual first, OCR fallback.', 'ok');
