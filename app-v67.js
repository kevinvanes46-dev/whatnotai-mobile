...
async function scanCurrentPhoto(){
  ...
  try{
    ...
    const headerText=headerScan?.data?.text || '';
    const trainerDetected=looksLikeTrainer(headerText);
    const trainerTitle=trainerDetected ? extractTrainerTitle(headerText) : '';
    ...
    const local=(displayName && canAutofill) ? findKnownLocal(displayName, collector?.number, inferredSet) : null;
    if(ocrResult){
      const pieces=[];
      if(trainerDetected){
    ...
    if(displayName && canAutofill){
      setStatus(`Scan: ${displayName}${collector?` #${collector.number}`:''}. Controleer naam, nummer, set en staat.`, 'ok');
...
bind();
renderSaved();
setStatus('v67 · Guided Scan · DB 243.', 'ok');
