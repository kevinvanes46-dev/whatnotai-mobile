...
function parseQuick(){
  ...
  // v69: 1st Edition is een echte Cardmarket-offerfilter en mag nooit verdwijnen.
...
function buildUrl(){
  ...
  // v69: bij 1st Edition nooit een onzekere collectorcode forceren. Een fout nummer
...
// ---------------- Photo + Local OCR + Guided Scan v69 ----------------
...
function loadPhotoFile(file, options={}){
  ...
  // v69: een nieuwe kaartfoto mag NOOIT oude scanvelden meenemen.
...
function extractLikelySetTotal(text){
  // v69: soms leest OCR alleen de denominator (bv. 111 van 62/111).
  // Accepteer uitsluitend totalen die CardScout al eenduidig aan een set koppelt.
  const t=String(text||'').replace(/O/g,'0').replace(/[Il]/g,'1');
  const nums=[...t.matchAll(/\b(\d{2,3})\b/g)].map(m=>Number(m[1]));
  const hits=nums.filter(n => Object.prototype.hasOwnProperty.call(SET_TOTAL_HINTS,n));
...
async function detectFirstEditionStamp(worker, canvas, guided){
  // v69: de WOTC 1st Edition-stempel is klein. Scan meerdere gerichte zones
...
async function scanCurrentPhoto(){
  ...
  try{
    ...
    // v69: a second high-contrast pass over a wider top band catches names such as
    ...
    const denominatorHint = collector ? collector.total : extractLikelySetTotal(numberOcrText);
    let inferredSet=denominatorHint ? inferSetFromTotal(denominatorHint) : null;
    ...
    // v69: als de OCR de naam betrouwbaar heeft maar het minuscule kaartnummer mist,
    ...
    if(displayName && canAutofill){
      ...
      }else if(inferredSet && setCandidates.length===1){
        // Naam + eenduidige set wint van een duidelijk conflicterende OCR-numerator.
        localInference=setCandidates[0];
        if(collector && cleanNumber(localInference.number)!==cleanNumber(collector.number)) collectorRejected=true;
      }else if(!collector && allNameCandidates.length===1){
        localInference=allNameCandidates[0];
      }
      if(localInference && !inferredSet) inferredSet=localInference.set;
    }

    // v69: velden weerspiegelen uitsluitend DEZE scan. Nooit oude kaartdata laten staan.
    ...
    if(ocrResult){
      ...
      else if(collectorRejected && localInference) pieces.push(`<strong>Nummer:</strong> ${escapeHtml(localInference.number)} <small>(OCR ${escapeHtml(collector.number)} verworpen door naam+set-match)</small>`);
      ...
      if(inferredSet) pieces.push(`<strong>Set-hint:</strong> ${escapeHtml(EMBEDDED_DATA.sets[inferredSet]?.label || inferredSet)}${!collector && denominatorHint ? ' <small>(set-totaal gelezen)</small>' : ''}`);
...
bind();
renderSaved();
setStatus('v69 · First Ed Safe · DB 253.', 'ok');
